...
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_miss_probe.py` | Negative guard for the remaining high-amplitude Cb singleton / Cr all-but-one `0x2/0xd` chroma-AC complements; locks exact final-slice tails, one-frame FFmpeg miss signatures, byte-identical IDR frames, exact broad first-payload expected-SAD mutation classes, the narrow second-payload class `0x1a-0x1b`, and third/final-payload dead ends so the next arithmetic/context repair has a stable promotion target below literal byte patching |
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` | DEBUG_CABAC_P16X16 trace compare for the remaining high-amplitude `Cb0x2/Cr0xd` one-frame miss sign-family against the reciprocal strict-pass `Cb0xd/Cr0x2` lane; locks plane-local CABAC/chroma-AC counters and CAVLC suppression, plane-local CBF walks, exact coded-block residual bin trails, common P-slice emit-tail bit-buffer rows, sign-dependent first payload bytes (`bb ec f7` / `bb cc ff` vs `3a dd f5`), residual output chunks, first-payload arithmetic/context state, per-coded-block final payload context counts/states, and terminate pre-state so the next source repair stays below residual binarization and on CABAC arithmetic/output divergence |
