...
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` | DEBUG_CABAC_P16X16 trace compare for the remaining high-amplitude `Cb0x2/Cr0xd` one-frame miss sign-family against the reciprocal strict-pass `Cb0xd/Cr0x2` lane; locks plane-local CBF walks, sign-dependent first payload bytes (`bb ec f7` / `bb cc ff` vs `3a dd f5`), residual output chunks, first-payload arithmetic/context state, per-coded-block final payload context counts/states, and terminate pre-state so the next repair can target residual output-byte renormalization rather than another CBF selector guess |
