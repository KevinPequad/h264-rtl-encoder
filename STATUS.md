...
| `scripts/run_cabac_p16x16_chroma_cb_ac_queue_align_probe.py` | Staged arithmetic-core queue-alignment probe that keeps the canonical source untouched, changes only the isolated workspace CABAC core initial `cod_i_queue` from `-9` to `-8`, verifies all 15 Cb-only chroma-AC masks strict-decode two frames with byte-identical IDR and expected Cb-only SAD, and now locks the both-plane Cb+Cr AC regression that blocks promoting that global queue shift into source |
| `scripts/run_cabac_p16x16_chroma_cb_ac_first_payload_substitution_probe.py` | Bytestream-side exact first-payload substitution probe that mutates only the first CABAC residual byte after the locked `d0 08 08 6b` P-slice header; proves `0xeb->0x75` promotes all Cb-only AC masks while the dense Cb+Cr guard still strict-decodes, separating the useful first-byte correction from the non-committable global queue-shift side effect |
...
- 2026-05-30: added `scripts/run_cabac_p16x16_chroma_cb_ac_first_payload_substitution_probe.py`, a bytestream-side diagnostic that mutates only the first CABAC payload byte after the locked `d0 08 08 6b` P-slice header. Exact `0xeb -> 0x75` substitution promotes every Cb-only AC mask `0x1..0xf` to strict `768/768` decode with expected Cb-only SAD, and the dense Cb+Cr guard still strict-decodes with `U_SAD=256 V_SAD=256`; this separates the useful first-byte correction from the non-committable global queue-shift side effect, so the next fix should target first-payload generation while preserving later both-plane arithmetic/output state.
