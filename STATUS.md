...
- the CABAC P16x16 cross-plane chroma-AC gate now also guards the complete
  ...
  with exact FFmpeg signatures and final P-slice tails; the same focused gate
  now also mutates only the first residual payload byte for each remaining miss
  (`bit7`, `0x75`, and `0x6b`) and requires full two-frame FFmpeg decode with
  exact plane-local SAD, keeping the next repair target on CABAC
  arithmetic/output-byte generation rather than a scan/CBF ownership guess.
...
| `scripts/run_cabac_p16x16_luma_sparse_chroma_ac_residual_check.py` | Combined CABAC P16x16 luma plus sparse chroma-AC residual smoke gate; locks strict two-frame FFmpeg decode for all four single-plane Cb/Cr sparse quadrants, same-plane row/column two-block Cb/Cr sparse pairs, representative same-block Cb+Cr quadrants, both directions of row-adjacent and column-adjacent Cb+Cr singleton pairs, true opposite-diagonal Cb+Cr AC pairs, default/high-amplitude same-mask row/diagonal controls, default and high-amplitude all-but-one same-mask mirrors (`Cb0x7/Cr0x7`, `Cb0xB/Cr0xB`, `Cb0xD/Cr0xD`, and `Cb0xE/Cr0xE`), with `Cb0x7/Cr0x7` locked across the +/-32 sign matrix, reciprocal high-amplitude right-column/diagonal skew pairs (`Cb0x6/Cr0x9` and `Cb0x9/Cr0x6`) across the +/-32 sign matrix, mirrored high-amplitude three-plus-one edge complements, single-right/three-right (`Cb0xB/Cr0x4` and `Cb0x4/Cr0xB`), middle-left/middle-right (`Cb0xD/Cr0x2` and `Cb0x2/Cr0xD`), and corner-left/corner-right (`Cb0xE/Cr0x1` and `Cb0x1/Cr0xE`) all-but-one luma mirrors across the +/-32 sign matrix, and high-amplitude complementary split-row/column +/-32 sign-matrix pairs with luma residual present, plane-local CABAC chroma-AC counters, exact CAVLC suppression counts, exact final P-slice bytes, current decoded-plane metrics (`Y_SAD=2048`, active sparse chroma block at `SAD=64`, same-plane pair at `SAD=128`, or high-amplitude sparse groups at `SAD=512/768`), and per-4x4 chroma-block locality so aggregate SAD cannot hide wrong-quadrant sparse residual placement |
...
| `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` | Representative post-`cod_i_queue=-7` cross-plane Cb+Cr chroma-AC gate; locks sparse/sparse, bottom/top mirror, split-row, sparse singleton mirror gaps, orthogonal axis-pair, one-vs-two-block row/axis pair samples, same-mask controls, dense-Cb, dense-Cr, dense-both, bottom-row sparse+dense, asymmetric three-block plus reciprocal mirror, low/high-amplitude three-plus-one-block complements, same-quadrant all-but-one/singleton mirrors, mixed-sign all-but-one/singleton complements, prior wrong-plane mixed cases, reciprocal high-amplitude Cb/Cr cases, the former high-amplitude same-mask checker miss `Cb0x5/Cr0x5` at `+32/+32`, mixed-sign split-row/axis-pair guards, the full `Cb0x3/Cr0xC` split-row +/-32 sign matrix, and adjacent skew-pair high-amplitude sign partitions as strict two-frame FFmpeg decodes with exact plane-local SAD and exact generated final P-slice tails |
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_miss_probe.py` | Promotion/quadrant-complement gate for high-amplitude Cb singleton / Cr all-but-one chroma-AC complements plus reciprocal shared-bank controls; locks strict two-frame FFmpeg decode, exact final-slice tails, and expected plane-local SAD for all sixteen `+/-32` sign combinations across `0x1/0xe`, `0x2/0xd`, `0x4/0xb`, and `0x8/0x7`, plus reciprocal `0xe/0x1`, `0xd/0x2`, and `0x7/0x8` sign families |
| `scripts/run_cabac_p16x16_chroma_ac_b4_split_probe.py` | Historical staged diagnostic for the former high-amplitude reciprocal `Cb0xb/Cr0x4` complement; patches only temporary workspaces to add that mask pair to the scoped split-context predicate and to route the Cr-side, Cb-side, both-plane, or individual sig/last/level residual context classes through split/shared banks, preserving the rejected split-bank repair-path evidence that preceded the scoped CBF-walk source repair |
| `scripts/run_cabac_p16x16_chroma_ac_b4_first_payload_substitution_probe.py` | Historical bytestream-side diagnostic for the former high-amplitude reciprocal `Cb0xb/Cr0x4` complement; locks the pre-repair first residual payload bytes (`0xfe`/`0xff`) and the one-byte `0x75` strict-decode substitution evidence that scoped the eventual source repair away from testbench patching |
| `scripts/run_cabac_p16x16_chroma_ac_b4_first_payload_value_sweep.py` | Historical bounded byte-value sweep for the former high-amplitude reciprocal `Cb0xb/Cr0x4` complement; preserves the first-CABAC-payload strict-decode equivalence classes gathered before the scoped CBF-walk source repair |
| `scripts/run_cabac_p16x16_chroma_ac_b4_second_payload_value_sweep.py` | Historical bounded second-CABAC-payload byte-value sweep for the former high-amplitude reciprocal `Cb0xb/Cr0x4` complement; preserves evidence that no second-byte-only strict expected-SAD repair class existed before the scoped source repair |
| `scripts/run_cabac_p16x16_chroma_ac_b4_third_payload_value_sweep.py` | Historical bounded third-CABAC-payload byte-value sweep for the former high-amplitude reciprocal `Cb0xb/Cr0x4` complement; preserves evidence that no third-byte-only strict expected-SAD repair class existed before the scoped source repair |
| `scripts/run_cabac_p16x16_chroma_ac_b4_payload_boundary_probe.py` | Historical payload-boundary diagnostic for the former high-amplitude reciprocal `Cb0xb/Cr0x4` complement; preserves the three-post-header-payload-byte boundary evidence used to rule out later-byte repair paths before the scoped source repair |
| `scripts/run_cabac_p16x16_chroma_ac_b4_terminal_state_probe.py` | Historical DEBUG_CABAC terminal-state diagnostic for the former high-amplitude reciprocal `Cb0xb/Cr0x4` complement; preserves the pre-repair terminal arithmetic low/range evidence that narrowed the eventual source repair to the early payload arithmetic / CBF-walk interaction |
| `scripts/run_cabac_p16x16_chroma_ac_b4_trace_contrast_probe.py` | DEBUG_CABAC trace-contrast gate for the repaired high-amplitude reciprocal `Cb0xb/Cr0x4` complement against the promoted mirror `Cb0x4/Cr0xb`; locks B4 as strict two-frame FFmpeg-decodable through the scoped plane-local CBF walk with coded chroma-AC blocks `[0,1,3,6]`, no split payload blocks, empty high-bank payload summary, exact integrated `CABAC_CHROMA` ownership/counter summaries, CBF selector walk `[0,1,2,2,4,4,4,5]`, exact tails (`...6b7fcf7f7b` / `...6b7edef7fa`), and the mirror strict-decode split-bank guard with coded blocks `[2,4,5,7]`, exact high-bank payload summary `[(4,24,17,5),(5,24,17,5),(7,24,17,5)]`, and exact plane-local SAD |
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` | DEBUG_CABAC_P16X16 scoped-bank smoke for the promoted high-amplitude `Cb0x2/Cr0xd` and `Cb0x4/Cr0xb` sign-families against mixed-sign shared-bank controls plus reciprocal `Cb0xe/Cr0x1`, `Cb0xd/Cr0x2`, and `Cb0x7/Cr0x8` strict-pass controls; proves the target families use the high-bit Cr residual payload context bank while the adjacent/reciprocal controls remain on the historical shared bank |
| `scripts/run_cabac_p16x16_chroma_ac_checker_high_amp_sign_probe.py` | Focused complementary-checker high-amplitude sign-partition probe for `Cb0x5/Cr0xA` and `Cb0xA/Cr0x5`; locks all eight +/-32 endpoints as strict two-frame FFmpeg decodes with exact plane-local SAD and final P-slice tails, including the repaired Cr-negative directions (`...6b3acc6e` / `...6b3aec7e`) |
| `scripts/run_cabac_p16x16_chroma_ac_same_mask_high_amp_probe.py` | Focused high-amplitude same-mask Cb+Cr chroma-AC probe; locks the promoted `Cb0x5/Cr0x5` sign matrix, Cb-positive `Cb0x7/Cr0x7`, plus strict `Cb0xD/Cr0xD` and `Cb0xE/Cr0xE` rows, while preserving exact one-frame FFmpeg miss signatures for the remaining all-but-one same-mask rows and proving `bit7`/`0x75`/`0x6b` first-payload-only mutations promote those misses with exact plane-local SAD so future repairs cannot overclaim this lane |
| `scripts/run_cabac_p16x16_chroma_ac_checker_cbf_walk_rejection_probe.py` | Staged diagnostic that patches only an isolated workspace to put the complementary-checker high-amplitude families on the plane-local CBF-walk path, then proves that broad Cr-negative-gated selector expansion regresses the already-strict `Cb0x5/Cr0xA` Cr-positive control (`...6b7fff -> ...6bbecf`, FFmpeg `bytestream -21`) even though representative Cr-negative rows decode under that patch, preserving the checked-in narrower split-payload repair boundary |
...
| `scripts/run_cabac_p16x16_chroma_cb_ac_arith_trace_probe.sh` | DEBUG_CABAC_P16X16 arithmetic trace probe for all 15 nonzero Cb-only 2x2 AC masks; locks strict two-frame FFmpeg decode, CBF arithmetic/context trails, P-slice prefix emission and bit-buffer rows, residual output/emit byte chunks, stream tails, terminate pre-state, first-payload state, IDR-frame integrity, and exact Cb-only decoded-plane SAD |
...
| `scripts/run_cabac_p16x16_chroma_cb_ac_queue_align_probe.py` | Staged arithmetic-core queue-alignment probe that keeps the canonical source untouched, first locks the checked-in `-7` source baseline across all 15 Cb-only and all 15 Cr-only chroma-AC masks plus the dense both-plane guard, then changes only the isolated workspace CABAC core initial `cod_i_queue` from `-7` to `-8`, verifies all 15 Cb-only and all 15 Cr-only chroma-AC masks strict-decode two frames with byte-identical IDR and expected plane-local SAD, locks representative mixed Cb+Cr mask pairs as strict under that candidate, and locks the remaining dense Cb+Cr AC regression final-slice mutation (`...6b7acc -> ...6bf599`, first residual byte `0x7a->0xf5` plus trailing `0x99`) while proving bytestream-side first-payload substitution (`0xf5->0xeb/0x75`) restores strict two-frame decode and trimming only `0x99` does not |
| `scripts/run_cabac_p16x16_chroma_ac_scoped_queue_probe.py` | Refreshed staged scoped queue-init probe for the checked-in `cod_i_queue=-7` source; adds an isolated `init_queue` port and proves a slice-start conditional on current chroma-AC scan contents falls back to the -7 baseline, keeping sparse Cb masks `0x1`/`0x2`, mask `0x3`, and dense Cb+Cr strict with locked final slices rather than providing a source promotion path |
...
- 2026-05-31: extended the cross-plane chroma-AC all-but-one/singleton guard with mixed-sign amplitude variants for `Cb/Cr 0x1/0xe`, `0xe/0x1`, and `0xd/0x2`. The new cases strict-decode two FFmpeg frames with exact plane-local SAD (`U/V 256/768` or `768/256`) and lock their final P-slice tails, while a sampled `Cb/Cr 0x2/0xd` both-high complement still fails strict FFmpeg with `bytestream -16` and remains the next arithmetic/context target.
- 2026-05-31: extended the default-amplitude cross-plane chroma-AC all-but-one/singleton guard with same-quadrant mirrors `Cb/Cr 0xb/0x4`, `0x4/0xb`, `0xd/0x2`, and `0x2/0xd`. These strict-decode two FFmpeg frames with exact plane-local SAD (`U/V 192/64` or `64/192`) and lock their generated final P-slice tails, separating the passing low-amplitude quadrant mirrors from the remaining high-amplitude `0x2/0xd` arithmetic miss.
- 2026-05-31: extended the cross-plane chroma-AC all-but-one/singleton guard with sampled all-negative mixed-sign complements for `Cb/Cr 0x1/0xe`, `0xe/0x1`, and `0xd/0x2`, plus the reciprocal `0xd/0x2` Cb-high/Cr-low case. These strict-decode two FFmpeg frames with exact plane-local SAD (`U/V 256/768` or `768/256`) and lock final P-slice tails, while the sampled `Cb/Cr 0x2/0xd` high-amplitude complements still fail strict FFmpeg (`bytestream -16/-26`) and remain the next arithmetic/context target.
- 2026-05-31: added `scripts/run_cabac_p16x16_chroma_ac_high_amp_miss_probe.py`, a bounded negative guard for the remaining high-amplitude `Cb/Cr 0x2/0xd` complements. It locks the exact generated final-slice tails (`...bbbecf7` for Cr-high, `...bbbccff` for Cr-low), strict one-frame FFmpeg miss signatures (`bytestream -16/-26`), and byte-identical IDR frames so a later CABAC arithmetic/context repair has a stable promotion target instead of rediscovering the failure shape.
- 2026-06-01: tightened `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` so the remaining high-amplitude `Cb0x2/Cr0xd` miss and reciprocal strict-pass lane now lock the full residual `CABACBITS` byte-chunk sequence, not just final slice tails, emitted payload bytes, CBF order, and first payload arithmetic context. The failing lane is now pinned at chunks `(blk,byte,bit_cnt) = (1,05,40),(1,44,48),(1,1e,56),(1,80,64),(4,09,72),(4,84,80),(4,03,88),(4,0e,96),(6,d6,104),(6,e9,112),(6,0f,120),(7,b3,0),(7,a4,8),(7,b6,16)`, while the reciprocal pass locks `(0,1a,40),(0,04,48),(0,bc,56),(0,71,64),(2,4a,72),(2,c4,80),(2,6b,88),(2,e5,96),(3,ad,104),(3,ad,112),(3,ae,120),(5,12,0),(5,99,8),(5,b1,16)`. This keeps the next repair focused on residual output-byte arithmetic/renormalization after the divergent CBF walks.
- 2026-06-01: extended the same high-amplitude trace probe from the single `+32/+32` miss into the full remaining `Cb0x2/Cr0xd` sign-family, including sign-dependent residual chunks, first-payload bytes (`bb ec f7` for Cr-high and `bb cc ff` for Cr-low), first-payload arithmetic contexts, and terminate pre-state. The parser now tolerates known Verilator/TB stdout interleaving around `[CABACTERM]`, keeping the new terminate-state locks stable while the CABAC arithmetic repair target remains open.
- 2026-06-01: tightened `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` again with exact CHRAC_CBF context-state/arithmetic trails for the four remaining `Cb0x2/Cr0xd` high-amplitude sign-family misses and the reciprocal `Cb0xd/Cr0x2` strict-pass lane. The CBF value order was already locked; this now pins the selector, state-in/out, low/range/queue, pending byte, and per-plane bank split before the first residual payload, narrowing the next source repair to CABAC arithmetic/output divergence rather than another CBF selector rewrite.
- 2026-06-01: promoted the cross-plane chroma-AC high-amplitude same-quadrant mirrors in `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py`. The gate now locks strict two-frame FFmpeg decode, exact plane-local SAD, and final P-slice tails for `Cb/Cr 0x2/0xd` at `+32/+32` and `-32/-32`, plus `0xb/0x4` and `0x4/0xb` at both signs, closing the formerly isolated `0x2/0xd` high-amplitude complement miss in the bounded cross-plane regression lane.
- 2026-06-01: tightened `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` again with exact coded-block residual bin-trail locks for the remaining high-amplitude `Cb0x2/Cr0xd` sign-family misses and reciprocal `Cb0xd/Cr0x2` strict-pass lane. The new lock proves the significant/last/level/bypass sequence is stable and only the expected `+32`/`-32` bypass sign bits differ, keeping the next repair below residual binarization and on CABAC arithmetic/renormalization/output-byte state.
- 2026-06-01: tightened `scripts/audit_cabac_chroma_residual_scaffold.py` so the static scaffold audit now requires the high-amplitude cross-plane `Cb0x2/Cr0xd` payload mutation classes and debug-trace arithmetic locks. This keeps the remaining miss target pinned across the one-byte first/second-payload expected-SAD classes, the third-payload dead end, the fail/pass residual chunk sequences, and the first-payload/terminate CABAC arithmetic state before attempting the next source repair.
- 2026-06-01: promoted the former high-amplitude `Cb0x2/Cr0xd` cross-plane chroma-AC miss family by adding a scoped Cr residual payload context bank in `rtl/h264_bitstream.v` for exactly the Cb-singleton / Cr-all-but-one mask pair. `scripts/run_cabac_p16x16_chroma_ac_high_amp_miss_probe.py` now locks strict two-frame FFmpeg decode and exact plane-local SAD for all four `+/-32` sign combinations (`...b3aec7fe6`, `...b3aec7f7c`, `...b3adefda6`, `...b3adefdfc`), while `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` verifies the target family uses the split Cr payload bank and the reciprocal `Cb0xd/Cr0x2` control remains shared. The cross-plane gate was refreshed for the default-amplitude `Cb0x2/Cr0xd` tail, and the scaffold audit now requires the scoped-bank RTL and promoted probes.
- 2026-06-01: tightened `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` to also run shared-bank high-amplitude controls for `Cb0x1/Cr0xe` and `Cb0x8/Cr0x7` at positive and negative `+/-32` endpoints. The trace gate now proves the scoped Cr payload context split remains limited to the repaired `Cb0x2/Cr0xd` and `Cb0x4/Cr0xb` families while adjacent promoted all-but-one/singleton complements strict-decode through the historical shared bank; the scaffold audit locks this split-vs-shared boundary.
- 2026-06-01: extended the high-amplitude trace shared-bank controls with the mixed-sign `Cb0x1/Cr0xe` and `Cb0x8/Cr0x7` endpoints. These adjacent all-but-one/singleton complement families still strict-decode with exact final-slice tails while keeping `split_bank=False`, so the scoped Cr residual payload bank remains confined to the repaired `Cb0x2/Cr0xd` and `Cb0x4/Cr0xb` sign families instead of leaking by sign polarity.
- 2026-06-01: extended the same `DEBUG_CABAC_P16X16` high-amplitude trace gate with reciprocal shared-bank controls for `Cb0xe/Cr0x1`, `Cb0xd/Cr0x2`, and `Cb0x7/Cr0x8` across the sampled `+/-32` endpoint matrix. The gate now locks strict two-frame FFmpeg decode, exact final-slice tails, and `split_bank=False` for those reciprocal families while the repaired `Cb0x2/Cr0xd` and `Cb0x4/Cr0xb` families remain the only split-bank cases; a sampled `Cb0xb/Cr0x4` high-amplitude reciprocal still short-decodes and remains the next arithmetic/context target.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_b4_split_probe.py` from the original Cr-side split-predicate negative into a two-variant staged diagnostic. It now also routes the Cb side of the high-amplitude reciprocal `Cb0xb/Cr0x4` complement through the split payload bank in an isolated workspace and locks that all four `+/-32` sign combinations still short-decode (`bytestream -11/-23`, tails `...bfacdf5f7` / `...bfafe77f7`), so the next repair should target a deeper CABAC arithmetic/output-state difference instead of either simple split-bank direction.
- 2026-06-01: extended the same B4 split diagnostic with a third staged both-plane payload-bank variant. Routing both Cb and Cr residual payloads for high-amplitude reciprocal `Cb0xb/Cr0x4` through the split bank still short-decodes one FFmpeg frame for all four `+/-32` sign combinations (`corrupt decoded frame`, tails `...bfacdf577f4` / `...bfafe77f75d`), so the open repair is not a simple Cr-side, Cb-side, or both-plane reuse of the existing split context bank.
- 2026-06-01: tightened `scripts/run_cabac_p16x16_chroma_ac_b4_split_probe.py` so it first locks the unpatched high-amplitude reciprocal `Cb0xb/Cr0x4` shared-bank baseline before trying staged split-bank variants. The current source still short-decodes one FFmpeg frame for all four `+/-32` endpoints (`bytestream -4` for Cr-high tails `...bfedff5`, `corrupt decoded frame` for Cr-low tails `...bffef75`), keeping the open repair grounded in the exact source failure rather than only the patched-workspace negatives.
- 2026-06-01: added `scripts/run_cabac_p16x16_chroma_ac_b4_first_payload_value_sweep.py`, a bounded first-residual-payload byte sweep for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement. The current source still emits baseline first bytes `0xfe`/`0xff` outside the strict-decode classes, while `0x6b`/`0x75` remain promoted and the exact expected-SAD pass classes are locked at `174` values for Cr-high endpoints and `178` for Cr-low endpoints. This keeps the next source repair on CABAC arithmetic/renormalization/output-byte state rather than literal bytestream repair.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_cb_ac_arith_trace_probe.sh` from the representative Cb-only AC subset to the complete nonzero 2x2 Cb mask lattice. The gate now builds fixtures for masks `0x1..0xf`, strict-decodes all 15 through FFmpeg, and locks per-mask CBF arithmetic trails, CBF/payload order, emitted residual byte chunks, stream tails, terminate pre-state, first-payload state, byte-identical IDR, and exact Cb-only SAD (`64 * popcount(mask)`), keeping the post-`cod_i_queue=-7` Cb AC lane pinned while the next open target remains cross-plane high-amplitude arithmetic.
- 2026-06-01: added `scripts/run_cabac_p16x16_chroma_ac_b4_second_payload_value_sweep.py`, the second-residual-payload counterpart to the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` first-byte sweep. It locks baseline second bytes `0xdf`/`0xef` for the four `+/-32` endpoints and proves that mutating only that byte has an empty strict expected-SAD class, so the open repair stays on earlier CABAC arithmetic/renormalization/output-byte state rather than a later single-byte tail patch.
- 2026-06-01: added `scripts/run_cabac_p16x16_chroma_ac_b4_third_payload_value_sweep.py`, continuing the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` byte-boundary diagnostic to the third residual payload byte. It locks the baseline third bytes (`0xf5` for Cr-high endpoints and `0x75` for Cr-low endpoints) and proves mutating only that byte has an empty strict expected-SAD class for all four `+/-32` cases, keeping the next source repair focused on earlier CABAC arithmetic/renormalization/output-byte state rather than a later literal tail patch.
- 2026-06-01: added and tightened `scripts/run_cabac_p16x16_chroma_ac_b4_trace_contrast_probe.py`, a DEBUG_CABAC trace-contrast diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement against the promoted mirror `Cb0x4/Cr0xb`. It locks the open B4 lane as shared-bank one-frame misses with coded chroma-AC blocks `[0,1,3,6]`, exact first payloads `0xfe`/`0xff`, no split-payload blocks, final tails, and expected FFmpeg signatures, while the mirror strict-decodes through the split payload bank with coded blocks `[2,4,5,7]`, exact split-payload blocks `[4,5,7]`, final tails, and `U_SAD=256 V_SAD=768`; the scaffold audit now requires this contrast so the next source repair targets the asymmetric arithmetic/output-byte state rather than scan ownership or simple split-bank selection.
- 2026-06-01: reconciled the B4 status inventory with the repaired source state: `scripts/run_cabac_p16x16_chroma_ac_b4_trace_contrast_probe.py` now verifies `Cb0xb/Cr0x4` strict two-frame FFmpeg decode through the scoped plane-local CBF walk, with coded blocks `[0,1,3,6]`, no split payload blocks, selector walk `[0,1,2,2,4,4,4,5]`, exact repaired tails (`...6b7fcf7f7b` / `...6b7edef7fa`), and the `Cb0x4/Cr0xb` mirror split-bank guard. The older B4 split/payload/terminal probes are now documented as historical diagnostics for the pre-repair miss rather than current open blockers.
- 2026-06-01: promoted the remaining high-amplitude split-row `Cb0x3/Cr0xC` positive-Cb/negative-Cr sign directions by routing that exact mask pair through the literal plane-local chroma-AC CBF neighbour walk. The cross-plane gate now locks the full `Cb0x3/Cr0xC` +/-32 sign matrix as strict two-frame FFmpeg decode with exact plane-local SAD and new tails (`...6b7ede` for the formerly short signs), while `scripts/run_cabac_p16x16_chroma_ac_split_row_payload_probe.py` proves the old Cr-side split-payload-bank tweak is not the canonical repair because it changes the strict tail to `...6b7efd5f`.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` with high-amplitude skew-pair sign partition coverage for `Cb/Cr 0x3/0x5`, `0x5/0x3`, `0x6/0x9`, and `0x9/0x6`. The gate now locks the passing sign directions as strict two-frame FFmpeg decodes with exact plane-local SAD and final tails (`...6b3fcf`, `...6bbaff`, `...6b3eff`, `...6b7ffeef`, `...6b7ade`) while preserving six scoped one-frame expected-miss signatures (`...6bfbcf`, `...6bbfceef`, `...6bbefe`) for the remaining skew high-amplitude directions, keeping the next repair target on CABAC arithmetic/output-state rather than rediscovering the sign partition.
- 2026-06-01: promoted the remaining adjacent skew-pair high-amplitude sign directions by routing `Cb0x3/Cr0x5`, `Cb0x6/Cr0x9`, and `Cb0x9/Cr0x6` through the same narrow plane-local chroma-AC CBF neighbour walk used for the B4 and split-row repairs. The cross-plane gate now locks those six formerly one-frame miss endpoints as strict two-frame FFmpeg decodes with exact plane-local SAD and new/retimed tails (`...6b3add`, `...6b7bfd`, `...6b7bfeef`, `...6b3fdd7e`, `...6b7bef7e`), leaving no expected-miss directions in the bounded skew-pair high-amplitude gate; the scaffold audit now requires the scoped RTL predicates and promoted tails.
- 2026-06-01: added `scripts/run_cabac_p16x16_chroma_ac_checker_high_amp_sign_probe.py`, a focused complementary-checker high-amplitude sign-partition gate. It locks `Cb0x5/Cr0xA` and `Cb0xA/Cr0x5` Cr-positive +/-32 directions as strict two-frame FFmpeg decodes with exact plane-local SAD and final tails (`...6b7fff` / `...6b7bef`), while preserving the current Cr-negative directions as scoped one-frame expected misses (`bytestream -23` / `-15`, tails `...6bbbff` / `...6bbfef`). A staged attempt to add those checker masks to the literal plane-local CBF walk regressed existing strict directions, so the next repair should target a narrower arithmetic/output-state difference rather than broadening the current CBF-walk predicate.
- 2026-06-01: promoted the remaining complementary-checker high-amplitude `Cb0x5/Cr0xA` and `Cb0xA/Cr0x5` Cr-negative directions by routing those exact mask families through the split Cr residual payload bank only when the internal Cr negative-residual predicate is clear. `scripts/run_cabac_p16x16_chroma_ac_checker_high_amp_sign_probe.py` now locks all eight +/-32 sign endpoints as strict two-frame FFmpeg decodes with exact plane-local SAD, preserving the previous Cr-positive tails (`...6b7fff` / `...6b7bef`) and adding repaired Cr-negative tails (`...6b3acc6e` / `...6b3aec7e`); the scaffold audit now guards the scoped split-payload predicate.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` with eight default-amplitude two-block off-diagonal skew-pair regressions (`Cb/Cr 0x3/0x6`, `0x6/0x3`, `0x5/0x6`, `0x6/0x5`, `0x9/0xA`, `0xA/0x9`, `0xB/0xC`, and `0xC/0xB`). The gate now locks those mixed-plane Cb+Cr AC masks as strict two-frame FFmpeg decodes with exact plane-local SAD and final P-slice tails, widening the bounded cross-plane lattice without changing RTL.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` with four default-amplitude reciprocal three-block complements (`Cb/Cr 0x7/0xE`, `0xE/0x7`, `0xB/0xD`, and `0xD/0xB`). These cases strict-decode two FFmpeg frames with exact plane-local SAD (`U/V 192/192`) and locked final P-slice tails, widening the bounded cross-plane lattice without changing RTL.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` with eight default-amplitude sparse singleton / one-vs-two Cb+Cr AC mirror gaps (`Cb/Cr 0x1/0x4`, `0x1/0x8`, `0x2/0x8`, `0x8/0x1`, `0x2/0x5`, `0x5/0x2`, `0x4/0x5`, and `0x5/0x4`). The gate locks each as a strict two-frame FFmpeg decode with exact plane-local SAD and final P-slice tails, further filling the bounded cross-plane lattice without changing RTL.
- 2026-06-01: extended `scripts/run_cabac_p16x16_luma_sparse_chroma_ac_residual_check.py` with the mirrored high-amplitude three-plus-one edge complement `Cb0x8/Cr0x7` at `+32/+32`. The luma+sparse gate now locks that case as a strict two-frame FFmpeg decode with plane-local counters (`cb_ac_blocks=1 cr_ac_blocks=3`), exact final P-slice `0000000141d008086b3abeefbffc7f5f7dbfff6dff`, `cavlc_suppressed_bits=295`, and decoded-plane metrics `Y_SAD=2048 U_SAD=256 V_SAD=768`, complementing the existing `Cb0x7/Cr0x8` edge case.
- 2026-06-01: extended `scripts/run_cabac_p16x16_luma_sparse_chroma_ac_residual_check.py` with the high-amplitude single-right/three-right all-but-one mirror families `Cb0xB/Cr0x4` and `Cb0x4/Cr0xB` under luma residual across all four +/-32 sign combinations. The gate now locks strict two-frame FFmpeg decode, plane-local block locality (`U/V 768/256` and `256/768`), exact CAVLC suppression counts (`296/294/292` by sign partition), and final P-slice tails for those luma+chroma AC complements.
- 2026-06-01: extended `scripts/run_cabac_p16x16_luma_sparse_chroma_ac_residual_check.py` with the high-amplitude middle-left/middle-right all-but-one mirror families `Cb0xD/Cr0x2` and `Cb0x2/Cr0xD` under luma residual across all four +/-32 sign combinations. The gate now locks strict two-frame FFmpeg decode, plane-local block locality (`U/V 768/256` and `256/768`), exact CAVLC suppression counts (`296/294/292` by sign partition), and final P-slice tails for those additional luma+chroma AC complements.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` with six default-amplitude checker/three-block cross-row complements (`Cb/Cr 0xA/0xB`, `0xB/0xA`, `0xC/0xA`, `0xA/0xC`, `0x7/0xD`, and `0xD/0x7`). The gate now locks them as strict two-frame FFmpeg decodes with exact plane-local SAD and final P-slice tails, widening the bounded mixed-plane chroma-AC lattice without changing RTL.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` with four default-amplitude adjacent all-but-one/singleton complements (`Cb/Cr 0xE/0x2`, `0x2/0xE`, `0xE/0x4`, and `0x4/0xE`). These strict-decode two FFmpeg frames with exact plane-local SAD (`U/V 192/64` or `64/192`) and locked final P-slice tails, filling another bounded mixed-plane chroma-AC mirror gap without changing RTL.
- 2026-06-01: promoted the former high-amplitude same-mask checker miss `Cb0x5/Cr0x5` at `+32/+32` by adding that exact mask pair to the narrow plane-local chroma-AC CBF neighbour-walk path in `rtl/h264_bitstream.v`. The cross-plane gate now locks the case as strict two-frame FFmpeg decode with byte-identical IDR, exact plane-local SAD (`U_SAD=512 V_SAD=512`), and final P-slice tail `0000000141d008086b3add`; the bounded expected-miss table is now empty again for this gate.
- 2026-06-01: tightened `scripts/run_cabac_p16x16_chroma_ac_same_mask_high_amp_probe.py` so every remaining high-amplitude same-mask all-but-one miss now also has first-residual-payload mutation evidence. The gate preserves the baseline one-frame FFmpeg signatures/tails for Cb-negative `Cb0x7/Cr0x7`, all `Cb0xB/Cr0xB`, and the unpromoted `Cb0xD/Cr0xD`/`Cb0xE/Cr0xE` sign rows, then mutates only that first residual payload byte via `bit7`, `0x75`, and `0x6b`; all mutations strict-decode two frames with byte-identical IDR and exact plane-local `U_SAD=768 V_SAD=768`. This narrows the next source repair to CABAC arithmetic/output-byte generation for those same-mask rows without landing a literal bytestream patch.
- 2026-06-01: extended `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` with nine default-amplitude row/column mixed-plane complements (`Cb/Cr 0x1/0x9`, `0x1/0xA`, `0x2/0x9`, `0x4/0x6`, `0x6/0x1`, `0x8/0x5`, `0x9/0x1`, `0xA/0x1`, and `0xC/0x1`). The gate now locks each as a strict two-frame FFmpeg decode with exact plane-local SAD and generated final P-slice tails, continuing to fill the bounded cross-plane chroma-AC lattice without changing RTL.
