...
| `scripts/run_cabac_p16x16_chroma_ac_debug_compare.sh` | DEBUG_CABAC_P16X16 diagnostic compare for sparse chroma AC fail/pass pairs; locks CABACRES bin traces plus CABACCTX context-state update traces for Cr TL/TR and Cb mirror TL/BR so the next repair can target context-state/ordering behavior with known decode outcomes |
