...
| `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` | Representative post-`cod_i_queue=-7` cross-plane Cb+Cr chroma-AC gate; locks sparse/sparse, bottom/top mirror, split-row, orthogonal axis-pair, dense-Cb, dense-Cr, dense-both, bottom-row sparse+dense, prior wrong-plane mixed cases, reciprocal high-amplitude Cb/Cr cases, and mixed-sign split-row/axis-pair guards as strict two-frame FFmpeg decodes with exact plane-local SAD and exact generated final P-slice tails |
