...
| `scripts/run_cabac_p16x16_luma_sparse_chroma_ac_residual_check.py` | Combined CABAC P16x16 luma plus sparse chroma-AC residual smoke gate; locks strict two-frame FFmpeg decode for representative single-block Cb, single-block Cr, top-left/bottom-right same-block Cb+Cr AC cases, and opposite-diagonal Cb+Cr AC pairs, plane-local CABAC chroma-AC counters, exact CAVLC suppression counts, exact final P-slice bytes, current decoded-plane metrics (`Y_SAD=2048`, active sparse chroma block at `SAD=64`), and per-4x4 chroma-block locality so aggregate SAD cannot hide wrong-quadrant sparse residual placement |
