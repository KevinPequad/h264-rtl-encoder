...
- CABAC `P_L0_16x16` integration currently covers the strict zero-MVD/single-ref zero-CBP lane, a focused luma-only residual lane, reduced 4:2:0 chroma-only residual smoke cases for Cb/Cr DC-only and Cb/Cr DC+AC, and a combined luma+Cb+Cr AC residual smoke case; all are strict FFmpeg-decodable, with the combined lane explicitly locking the current luma/chroma decoded-plane metrics rather than claiming full reconstruction quality
