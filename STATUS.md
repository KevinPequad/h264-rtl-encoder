...
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_miss_probe.py` | Negative guard for the remaining high-amplitude Cb singleton / Cr all-but-one `0x2/0xd` chroma-AC complements; locks exact final-slice tails, one-frame FFmpeg miss signatures, and byte-identical IDR frames so the next arithmetic/context repair has a stable promotion target |
