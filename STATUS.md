...
| `scripts/run_cabac_p16x16_chroma_cb_ac_queue_align_probe.py` | Staged arithmetic-core probe that keeps the canonical source untouched, changes only the isolated workspace CABAC core initial `cod_i_queue` from `-9` to `-8`, and verifies all 15 Cb-only chroma-AC masks strict-decode two frames with byte-identical IDR and expected Cb-only SAD while the baseline partition remains locked |
