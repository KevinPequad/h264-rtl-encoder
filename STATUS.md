...
| `scripts/run_cabac_p16x16_chroma_ac_same_mask_high_amp_probe.py` | Focused high-amplitude same-mask Cb+Cr chroma-AC probe; locks the promoted `Cb0x5/Cr0x5` sign matrix plus strict `Cb0xD/Cr0xD` and `Cb0xE/Cr0xE` rows, while preserving exact one-frame FFmpeg miss signatures for the remaining all-but-one same-mask rows so future repairs cannot overclaim this lane |
