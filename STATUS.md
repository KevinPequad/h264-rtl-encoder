...
| `scripts/run_cabac_p16x16_chroma_cb_ac_shape_probe.sh` | Focused Cb-only chroma-AC coefficient-shape probe at the first nonzero quantized step; locks that top-row vertical shapes can strict-decode while some bottom-row/checker shapes still short-decode, and now pins exact final P-slice tails under the shared `d0 08 08 6b eb` header/first-payload prefix, narrowing the remaining sparse-Cb repair target from block placement or slice-boundary labeling to residual coefficient emission/order and arithmetic-tail state |
