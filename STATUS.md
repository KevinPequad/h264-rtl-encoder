...
| `scripts/run_cabac_p16x16_chroma_cr_ac_shape_probe.sh` | Cr-only chroma-AC coefficient-shape diagnostic; locks low-amplitude checker/vertical/horizontal shape strict decodes with exact Cr-only SAD across all four Cr AC blocks, plus the high-amplitude checker/axis-complement/diagonal strict/miss partition and exact final P-slice tails so the remaining repair target stays on residual coefficient level/suffix/order arithmetic rather than CBF selector, axis-side polarity, or P-slice boundary handling |
