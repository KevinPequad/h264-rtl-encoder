...
| `scripts/run_cabac_p16x16_chroma_ac_b4_split_probe.py` | Staged diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; patches only temporary workspaces to add that mask pair to the scoped split-context predicate and to route the Cb-side residual payload through the split bank, proving all four `+/-32` sign combinations still short-decode one FFmpeg frame under both variants and rejecting the simple Cr-side/Cb-side split-bank repair paths |
