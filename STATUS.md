...
| `scripts/run_cabac_p16x16_chroma_cb_ac_shape_probe.sh` | Focused Cb-only chroma-AC coefficient-shape probe at the first nonzero quantized step; locks that top-row vertical shapes can strict-decode while some bottom-row/checker shapes still short-decode, narrowing the remaining sparse-Cb repair target from block placement alone to residual coefficient emission/order and arithmetic state around the first AC payload |
