...
| `scripts/run_cabac_p16x16_chroma_ac_checker_high_amp_sign_probe.py` | Focused complementary-checker high-amplitude sign-partition probe for `Cb0x5/Cr0xA` and `Cb0xA/Cr0x5`; locks the Cr-positive +/-32 lanes as strict two-frame FFmpeg decodes with exact plane-local SAD, and locks the Cr-negative lanes as scoped one-frame expected misses (`bytestream -23` / `-15`) with exact final P-slice tails |
| `scripts/run_cabac_p16x16_chroma_ac_checker_cbf_walk_rejection_probe.py` | Staged diagnostic that patches only an isolated workspace to put the complementary-checker high-amplitude families on the plane-local CBF-walk path, then proves that Cr-negative-gated selector expansion regresses the already-strict `Cb0x5/Cr0xA` Cr-positive control (`...6b7fff -> ...6bbecf`, FFmpeg `bytestream -21`) while representative Cr-negative rows still short-decode with their canonical `bytestream -23` / `-15` signatures, keeping the remaining Cr-negative repair target on a narrower selector or CABAC arithmetic/renormalization |
