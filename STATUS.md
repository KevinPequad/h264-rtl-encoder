...
| `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` | Representative post-`cod_i_queue=-7` cross-plane Cb+Cr chroma-AC gate; locks sparse/sparse, bottom/top mirror, split-row, dense-Cb, dense-Cr, dense-both, prior wrong-plane mixed cases, reciprocal high-amplitude Cb/Cr cases, and an additional mixed-sign split-row guard as strict two-frame FFmpeg decodes with exact plane-local SAD and exact generated final P-slice tails |
