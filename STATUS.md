...
| `scripts/run_cabac_p16x16_chroma_cb_ac_shape_probe.sh` | Focused Cb-only chroma-AC coefficient-shape probe at the first nonzero quantized step; locks complementary checker parities plus vertical/horizontal shape mirrors across the 2x2 Cb AC block lattice, including strict vertical controls, horizontal top/bottom short-decodes, checker parity controls, and the bottom-right `horiz_bottom` `bytestream -18` tail, while pinning exact final P-slice tails under the shared `d0 08 08 6b eb` header/first-payload prefix; this narrows the remaining sparse-Cb repair target to residual coefficient emission/order or CABAC arithmetic-tail state instead of P-slice boundary placement |
