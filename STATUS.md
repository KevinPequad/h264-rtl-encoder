...
| `scripts/run_cabac_p16x16_chroma_ac_first_payload_value_sweep.py` | Representative first-CABAC-payload byte-value sweep for sparse Cb-only, Cr-only, and dense Cb+Cr chroma-AC controls; locks that the known `0xeb->0x75` / `0xeb->0x6b` promotions sit inside broad FFmpeg strict-decode equivalence classes rather than being unique byte signatures, and that baseline sparse Cb/Cr `0xeb` stays outside the short-case classes, so the source repair should target the arithmetic/renormalization boundary |
