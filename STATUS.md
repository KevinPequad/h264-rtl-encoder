...
- CABAC `P_L0_16x16` integration currently covers the strict zero-MVD/single-ref zero-CBP lane, a focused luma-only residual lane, and reduced 4:2:0 chroma-only residual smoke cases for Cb/Cr DC-only and Cb/Cr DC+AC, all strict FFmpeg-decodable with decoded-plane sanity checks
- the standalone CABAC residual scan-event helper and bin/context helper exist; the scan helper now has explicit luma, first/middle-event backpressure-hold, bounded chroma-DC, and bounded chroma-AC event coverage, the bin/context helper supports category-specific chroma DC/AC context bases plus an explicit coded-block-flag context-increment input, and encoder-top buffers chroma DC/AC scan vectors while preserving the `cbp_chroma=1` DC-only vs `cbp_chroma=2` DC+AC distinction; the integrated writer path now emits both coded-block-pattern chroma bins, initializes/updates the chroma DC/AC residual CABAC context-state banks, selects plane-local Cr AC and guarded sparse-Cb AC CBF context walks, reports per-plane chroma AC block counters, and skips luma residual category emission when a chroma-only residual macroblock has `cbp_luma=0`; the chroma residual gate now strict-decodes the four reduced Cb/Cr DC and DC+AC smoke streams; the focused sparse chroma AC probe keeps dense Cb, dense Cr, all sparse Cr single-block quadrants, dense both-plane AC, and bottom-row sparse Cb mirrors as strict-pass controls while top-row sparse Cb mirrors remain signature-locked misses (`bytestream -19` / `-21`); the Cb-only and Cr-only phase/polarity probes now also lock exact final P-slice tails and first-payload substitution behavior, with Cr-only remaining strict decode across the matching lattice; a staged queue-alignment probe now shows that changing only the CABAC core initial `cod_i_queue` from `-9` to `-8` promotes all 15 Cb-only and all 15 Cr-only AC masks to strict two-frame decode, and the both-plane guard regression is isolated to the first residual payload byte (`0xf5`) rather than the extra trailing byte alone because `0xf5->0xeb/0x75` bytestream substitutions restore strict decode; the next repair is landing equivalent plane-safe first-payload/arithmetic alignment without regressing the locked both-plane Cb+Cr AC guard
...
| `scripts/run_cabac_p16x16_chroma_residual_red_check.sh` | Legacy-named promoted gate proving integrated CABAC P16x16 Cb/Cr DC-only and Cb/Cr DC+AC residual smoke streams strict-decode with FFmpeg and decoded-plane sanity checks after running the chroma residual wiring audit |
| `scripts/run_cabac_p16x16_chroma_cr_ac_probe.sh` | Focused sparse-chroma-AC probe that keeps dense Cb, dense Cr, all sparse Cr single-block quadrants, dense both-plane AC, and bottom-row sparse Cb mirrors as strict-pass controls while locking the remaining top-row sparse Cb mirror FFmpeg miss signatures (`bytestream -19` / `-21`), expected CABAC plane MB counters, per-plane AC block counters, and decoded-plane sanity for pass controls |
| `scripts/run_cabac_p16x16_chroma_cr_ac_mask_probe.py` | Cr-only chroma-AC mask-lattice diagnostic covering all 15 nonzero 2x2 Cr AC block masks; locks the current strict masks (`0x1`, `0x2`, `0x4`, `0x6`, `0x8`, `0x9`, `0xf`) and exact one-frame FFmpeg miss signatures for multi-block masks (`0x3`, `0x5`, `0x7`, `0xa`, `0xb`, `0xc`, `0xd`, `0xe`) |
...
| `scripts/run_cabac_p16x16_chroma_cr_ac_shape_probe.sh` | Cr-only chroma-AC coefficient-shape diagnostic; locks low-amplitude checker/vertical/horizontal shape strict decodes with exact Cr-only SAD across all four Cr AC blocks, plus the high-amplitude checker/axis-complement/diagonal strict/miss partition, exact final P-slice tails, and `0xeb->0x75` / `0xeb->0x6b` first-payload substitution promotion/stability for every high-amplitude shape case so the remaining repair target stays on scoped CABAC first-payload/residual-tail arithmetic rather than CBF selector, axis-side polarity, or P-slice boundary handling |
...
| `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` | Representative cross-plane Cb+Cr chroma-AC first-payload diagnostic; locks sparse/sparse, bottom/top mirror, split-row, and dense/sparse mixed-plane baseline FFmpeg signatures, verifies the short baseline cases still preserve a byte-identical decoded IDR frame before failing on the P residual, and verifies exact `0xeb->0x75` plus bit7 `0xeb->0x6b` first-payload substitutions promote the sparse mixed-plane cases and the `Cb/Cr 0xe/0x1` strict-but-wrong-plane quality guard to strict two-frame decode with expected plane-local SAD while preserving strict dense-Cb/sparse-Cr, bottom/top, and dense-both controls |
...
| `scripts/run_cabac_p16x16_chroma_ac_first_payload_value_sweep.py` | Representative first-CABAC-payload byte-value sweep for sparse Cb-only, Cr-only, dense Cb+Cr, and mixed-plane strict-but-wrong-quality chroma-AC controls; locks that the known `0xeb->0x75` / `0xeb->0x6b` promotions sit inside broad FFmpeg strict-decode / quality-repair equivalence classes rather than being unique byte signatures, and that baseline sparse Cb/Cr `0xeb` stays outside the short/quality-repair classes, so the source repair should target the arithmetic/renormalization boundary |
| `scripts/run_cabac_p16x16_chroma_cb_ac_shape_probe.sh` | Focused Cb-only chroma-AC coefficient-shape probe; locks complementary checker parities plus a complete vertical/horizontal shape sweep across the 2x2 Cb AC block lattice, and now locks high-amplitude checker, diagonal main/anti, and vertical/horizontal axis shapes on all four blocks as one-frame misses with exact final P-slice tails under the shared generated `d0 08 08 6b eb` header/first-payload prefix while proving `0xeb->0x75` and bit7 `0xeb->0x6b` first-payload substitutions promote/stabilize every high-amplitude shape with exact Cb-only SAD; this narrows the remaining sparse-Cb repair target to scoped CABAC first-payload/residual-tail arithmetic rather than only block placement, sparse CBF context selection, low-amplitude checker parity/quantizer cutoff, or the P-slice boundary |
...
| `scripts/audit_cabac_chroma_residual_scaffold.py` | Static audit that locks the CABAC chroma residual CBP, scan-buffer, context-base, guarded Cb/Cr AC CBF selection, context-state dispatch, category-scheduling wiring, decoded-plane sanity checks, strict Cb/Cr DC+AC gate promotion, sparse chroma AC pass/miss probe coverage, dense Cb/both-plane AC strict-pass controls, and per-plane chroma AC block counter reporting |
