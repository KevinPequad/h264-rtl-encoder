...
| `scripts/run_cabac_p16x16_chroma_ac_scoped_queue_probe.py` | Staged scoped queue-init probe that adds a temporary bitstream-selected `init_queue` port and proves choosing `-8` from current chroma-AC scan contents behaves like baseline at slice start: sparse Cb masks `0x1`/`0x2` remain one-frame FFmpeg misses while Cb top-pair and dense Cb+Cr controls stay strict, ruling out a simple slice-start conditional on MB residual inputs as the source promotion path |
