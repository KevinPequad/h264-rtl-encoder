...
- CABAC `P_L0_16x16` integration currently covers the strict zero-MVD/single-ref zero-CBP lane, a focused luma-only residual lane, and reduced 4:2:0 chroma-only residual smoke cases for Cb DC-only, Cb DC+AC, and Cr DC-only, all strict FFmpeg-decodable
- the standalone CABAC residual scan-event helper and bin/context helper exist; the scan helper now has explicit luma, first/middle-event backpressure-hold, bounded chroma-DC, and bounded chroma-AC event coverage, the bin/context helper supports category-specific chroma DC/AC context bases plus an explicit coded-block-flag context-increment input, and encoder-top buffers chroma DC/AC scan vectors while preserving the `cbp_chroma=1` DC-only vs `cbp_chroma=2` DC+AC distinction; the integrated writer path now emits both coded-block-pattern chroma bins, initializes/updates the chroma DC/AC residual CABAC context-state banks, keeps the reduced Cb chroma AC residual coded-block-flag lane at the category base context while selecting a plane-local Cr AC CBF context from prior Cr blocks, and skips luma residual category emission when a chroma-only residual macroblock has `cbp_luma=0`; the chroma residual gate now always generates the Cr DC+AC fixture/probe, includes Cb-vs-Cr AC plane counters in the RTL summary, checks decoded-plane sanity for the strict-pass Cb/Cr smoke cases, and locks the current Cr AC expected-miss behavior, including decoder-success/short-raw-output cases; a focused Cr AC probe now reproduces the blocker across checker, every single-block Cr-only quadrant, both-plane AC, and matched sparse Cb-only single-block mirror shapes as decoder-success/short-output cases (384/768 decoded bytes) while preserving the expected plane counters; a static audit now guards the current Cb base-CBF lane because directly promoting Cb to the same plane-local CBF selector trips the dense Cb AC strict-pass control at `bytestream -6`, so sparse chroma AC residual decode remains the next chroma expansion item
...
| `scripts/run_cabac_p16x16_chroma_residual_red_check.sh` | Legacy-named promoted gate proving integrated CABAC P16x16 Cb DC-only, Cb DC+AC, and Cr DC-only residual smoke streams strict-decode with FFmpeg and decoded-plane sanity checks; also runs the chroma residual wiring audit and now keeps the next Cr DC+AC target covered by default as a passing expected-miss diagnostic for the current Cr AC short-decode blocker; set `CABAC_CHROMA_INCLUDE_CR_AC=1` to require strict Cr AC promotion |
| `scripts/run_cabac_p16x16_chroma_cr_ac_probe.sh` | Focused expected-miss probe for the current sparse chroma AC strict-decode blocker across Cr-only checker/all single-block quadrants, both-plane AC, and matched Cb-only single-block mirror fixtures, with a dense Cb-only strict-pass control; locks the expected CABAC plane MB counters, per-plane AC block counters, FFmpeg pass/miss signatures, and decoded-plane sanity for the dense Cb pass control while the strict gate is still red |
...
| `scripts/run_cabac_p16x16_chroma_cb_ac_arith_trace_probe.sh` | DEBUG_CABAC_P16X16 arithmetic trace probe for representative Cb-only AC masks; locks fail/pass decoded byte counts, FFmpeg signatures, CBF arithmetic/context trails, P-slice prefix emission and bit-buffer rows, residual output/emit byte chunks, stream tails, terminate pre-state, first-payload state, IDR-frame integrity, and Cb-only decoded-plane sanity for strict-pass controls |
| `scripts/run_cabac_p16x16_chroma_cb_ac_tail_bitflip_probe.sh` | Focused sparse-Cb AC payload mutation probe; locks the common pre-residual CABAC prefix-byte bit flips separately from residual-byte bit flips that can make the currently short masks strict-decode with Cb-only decoded deltas |
...
| `scripts/run_cabac_p16x16_chroma_cb_ac_neighbor_cbf_probe.py` | Staged diagnostic probe that replaces the current sparse-Cb-only synthetic CBF walk with direct plane-local-neighbor CBF derivation in a temporary workspace and locks that representative failing masks plus former green controls all remain one-frame FFmpeg misses, rejecting that simple selector restoration as the repair path |
| `scripts/run_cabac_p16x16_residual_red_check.sh` | Legacy alias for the promoted luma residual GREEN gate |
| `scripts/audit_cabac_chroma_residual_scaffold.py` | Static audit that locks the CABAC chroma residual CBP, scan-buffer, context-base/Cr-AC-CBF selection plus the current Cb base-CBF guard, context-state dispatch, category-scheduling wiring, decoded-plane sanity checks, the current Cr AC / both-plane AC expected-miss fixture/counter coverage, all sparse single-quadrant Cb/Cr expected-miss coverage, dense Cb AC strict-pass control plus decoded-plane sanity, and per-plane chroma AC block counter reporting |
...
- 2026-05-30: added `DEBUG_CABAC_P16X16` byte-emission tracing in `rtl/h264_bitstream.v` and tightened `scripts/run_cabac_p16x16_chroma_cb_ac_arith_trace_probe.sh` to lock the common P-slice prefix emission (`d0 08 08 6b`) separately from the residual emitted byte tail; sparse masks `0x1`, `0x2`, and `0xc` remain one-frame `384/768` misses while `0x3`, `0x4`, and `0x8` remain full `768/768` controls.
