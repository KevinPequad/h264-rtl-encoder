...
| `scripts/run_cabac_p16x16_chroma_ac_cross_plane_first_payload_probe.py` | Representative cross-plane Cb+Cr chroma-AC first-payload diagnostic; locks sparse/sparse, bottom/top mirror, split-row, and dense/sparse mixed-plane baseline FFmpeg signatures, and verifies exact `0xeb->0x75` plus bit7 `0xeb->0x6b` first-payload substitutions promote the sparse mixed-plane cases to strict two-frame decode with expected plane-local SAD while preserving strict dense-Cb/sparse-Cr, bottom/top, and dense-both controls |
