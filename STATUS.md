...
| `scripts/audit_cabac_chroma_residual_scaffold.py` | Static audit that locks the CABAC chroma residual CBP, scan-buffer, context-base, guarded Cb/Cr AC CBF selection, context-state dispatch, category-scheduling wiring, decoded-plane sanity checks, strict Cb/Cr DC+AC gate promotion, promoted Cb/Cr chroma-AC mask-lattice gates, dense Cb/both-plane AC strict-pass controls, and per-plane chroma AC block counter reporting |
