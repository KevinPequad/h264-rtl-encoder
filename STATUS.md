...
| `scripts/run_cabac_p16x16_chroma_ac_first_payload_value_sweep.py` | Representative first-CABAC-payload byte-value sweep for sparse Cb-only and dense Cb+Cr chroma-AC controls; locks that the known `0xeb->0x75` / `0xeb->0x6b` promotions sit inside a broad FFmpeg strict-decode equivalence class rather than being unique byte signatures, so the source repair should target the arithmetic/renormalization boundary that leaves sparse Cb baseline `0xeb` outside that class |
