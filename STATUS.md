...
| `scripts/run_cabac_p16x16_chroma_ac_b4_split_probe.py` | Staged diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; patches only a temporary workspace to add that mask pair to the scoped split-context predicate and proves all four `+/-32` sign combinations still short-decode one FFmpeg frame, rejecting the naive predicate-extension repair path |
