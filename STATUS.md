...
- the standalone CABAC residual scan-event helper and bin/context helper exist; the scan helper now has explicit luma, bounded chroma-DC, and bounded chroma-AC event coverage, the bin/context helper supports category-specific chroma DC/AC context bases, and encoder-top buffers chroma DC/AC scan vectors while preserving the `cbp_chroma=1` DC-only vs `cbp_chroma=2` DC+AC distinction; the integrated writer path now emits both coded-block-pattern chroma bins, initializes/updates the chroma DC/AC residual CABAC context-state banks, and skips luma residual category emission when a chroma-only residual macroblock has `cbp_luma=0`; the chroma residual gate now generates an optional Cr DC+AC fixture/probe via `CABAC_CHROMA_INCLUDE_CR_AC=1`, and strict Cr AC residual decode remains the next chroma expansion item
