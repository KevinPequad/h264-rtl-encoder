...
- the CABAC P16x16 cross-plane chroma-AC gate now also guards default-amplitude complementary checker masks `Cb0x5/Cr0xA` and `Cb0xA/Cr0x5`, low-amplitude and high-amplitude three-plus-one-block asymmetric complements `Cb0x7/Cr0x8`/`Cb0x8/Cr0x7`, default-amplitude same-quadrant all-but-one/singleton mirrors `Cb0xB/Cr0x4`, `Cb0x4/Cr0xB`, `Cb0xD/Cr0x2`, and `Cb0x2/Cr0xD`, plus mixed-sign all-but-one/singleton complements `Cb0x1/Cr0xE`, `Cb0xE/Cr0x1`, and `Cb0xD/Cr0x2` including sampled all-negative variants; the former high-amplitude `Cb0x2/Cr0xD` and `Cb0x4/Cr0xB` miss families now strict-decode via a scoped Cr residual payload context bank while reciprocal/dense guards stay on the historical shared bank; the staged `Cb0xB/Cr0x4` high-amplitude split-context diagnostic now rejects simply adding that reciprocal complement to the same scoped-bank predicate, moving it to the Cb-side/both-plane bank, or splitting only sig/last/level context classes because all four +/-32 sign cases still one-frame FFmpeg miss; the new DEBUG terminal-state probe locks those same `Cb0xB/Cr0x4` endpoint misses to exact three-byte final payloads on the shared residual context bank, with terminal arithmetic `ari_range=0x270` and `ari_low` partitioned by Cr sign (`0x19a`/`0x17e`), keeping the next source repair focused on first-payload arithmetic/renormalization rather than later bytes or bank routing
...
| `scripts/run_cabac_p16x16_chroma_ac_b4_split_probe.py` | Staged diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; patches only temporary workspaces to add that mask pair to the scoped split-context predicate and to route the Cr-side, Cb-side, both-plane, or individual sig/last/level residual context classes through split/shared banks, proving all four `+/-32` sign combinations still short-decode one FFmpeg frame under every variant and rejecting these simple split-bank repair paths |
| `scripts/run_cabac_p16x16_chroma_ac_b4_first_payload_substitution_probe.py` | Bytestream-side diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; locks the checked-in first residual payload bytes (`0xfe`/`0xff`) as one-frame FFmpeg misses and proves changing only that payload byte to `0x75` strict-decodes all four `+/-32` cases with exact Cb/Cr SAD, scoping the next source repair to the first-payload arithmetic/renormalization state rather than CBF ordering or testbench repair |
| `scripts/run_cabac_p16x16_chroma_ac_b4_first_payload_value_sweep.py` | Bounded byte-value sweep for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; mutates only the first CABAC residual payload byte and locks the exact strict-decode equivalence classes for all four `+/-32` endpoints, keeping the checked-in `0xfe`/`0xff` first bytes outside the class while preserving known `0x6b`/`0x75` promoted controls |
| `scripts/run_cabac_p16x16_chroma_ac_b4_second_payload_value_sweep.py` | Bounded second-CABAC-payload byte-value sweep for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; mutates only the second residual payload byte after the locked first byte and proves the strict expected-SAD class is empty for all four `+/-32` endpoints |
| `scripts/run_cabac_p16x16_chroma_ac_b4_third_payload_value_sweep.py` | Bounded third-CABAC-payload byte-value sweep for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; mutates only the third residual payload byte after locked first/second bytes and proves the strict expected-SAD class is empty for all four `+/-32` endpoints |
| `scripts/run_cabac_p16x16_chroma_ac_b4_payload_boundary_probe.py` | Payload-boundary diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; locks the four checked-in `+/-32` endpoint streams as exactly three post-header CABAC payload bytes with no generated fourth payload byte to sweep, keeping the next repair scoped to earlier arithmetic/renormalization state |
| `scripts/run_cabac_p16x16_chroma_ac_b4_terminal_state_probe.py` | DEBUG_CABAC terminal-state diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement; locks all four checked-in endpoint streams as shared-bank one-frame misses with exact three-byte final payloads and terminal arithmetic low/range state (`ari_low=0x19a/0x17e`, `ari_range=0x270`) so the next repair can target first-payload arithmetic/renormalization state directly |
| `scripts/run_cabac_p16x16_chroma_ac_b4_trace_contrast_probe.py` | DEBUG_CABAC trace-contrast diagnostic for the open high-amplitude reciprocal `Cb0xb/Cr0x4` complement against the promoted mirror `Cb0x4/Cr0xb`; locks B4 as shared-bank short-decodes with coded chroma-AC blocks `[0,1,3,6]` and first payloads `0xfe/0xff`, while the mirror strict-decodes through the split payload bank with coded blocks `[2,4,5,7]` and exact plane-local SAD |
