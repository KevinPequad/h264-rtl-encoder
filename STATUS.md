...
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_miss_probe.py` | Promotion gate for the former high-amplitude Cb singleton / Cr all-but-one `0x2/0xd` and `0x4/0xb` chroma-AC complements; locks strict two-frame FFmpeg decode, exact final-slice tails, and expected plane-local SAD for all eight `+/-32` sign combinations |
