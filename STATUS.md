...
- 2026-06-01: added `scripts/run_cabac_p16x16_chroma_ac_b4_trace_contrast_probe.py`, a DEBUG_CABAC trace-contrast diagnostic for the remaining high-amplitude reciprocal `Cb0xb/Cr0x4` complement against the promoted mirror `Cb0x4/Cr0xb`. It locks the open B4 lane as shared-bank one-frame misses with coded chroma-AC blocks `[0,1,3,6]`, exact first payloads `0xfe/0xff`, final tails, and expected FFmpeg signatures, while the mirror strict-decodes through the split payload bank with coded blocks `[2,4,5,7]`, exact final tails, and `U_SAD=256 V_SAD=768`; the scaffold audit now requires this contrast so the next source repair targets the asymmetric arithmetic/output-byte state rather than scan ownership or simple split-bank selection.
