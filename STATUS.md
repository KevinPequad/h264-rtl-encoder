...
| `scripts/run_cabac_p16x16_chroma_ac_high_amp_trace_probe.py` | DEBUG_CABAC_P16X16 trace compare for the remaining high-amplitude `Cb0x2/Cr0xd +32/+32` one-frame miss against the reciprocal strict-pass `Cb0xd/Cr0x2` lane; locks plane-local CBF walks, first payload bytes (`bb ec f7` vs `3a dd f5`), residual output chunks, first-payload arithmetic/context state, and terminate pre-state so the next repair can target residual output-byte renormalization rather than another CBF selector guess |
