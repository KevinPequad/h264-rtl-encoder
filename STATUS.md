...
| `scripts/run_cabac_p16x16_chroma_ac_checker_cbf_walk_rejection_probe.py` | Staged diagnostic that patches only an isolated workspace to put the complementary-checker high-amplitude families on the plane-local CBF-walk path, then proves that broad selector expansion regresses the already-strict `Cb0x5/Cr0xA` Cr-positive control (`...6b7fff -> ...6bbecf`, FFmpeg `bytestream -21`), keeping the remaining Cr-negative repair target on a narrower selector or CABAC arithmetic/renormalization |
