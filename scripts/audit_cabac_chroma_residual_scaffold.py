...
CHECKS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        r"cod_i_queue=-7.*?EXPECTED_HEADER_TAIL\s*=\s*0x6B.*?TAILS\s*=\s*\{.*?\(0x1,\s*0x1\):\s*\"0000000141d008086b3acbb8b517a9\".*?\(0x6,\s*0x9\):\s*\"0000000141d008086b3acc6941a435899e104b1c8a00\".*?\(0x9,\s*0x6\):\s*\"0000000141d008086b3acc68c9ae34745f6b095dfa00\".*?\(0xF,\s*0xF\):\s*\"0000000141d008086b7acc\".*?\(0xE,\s*0x1\):\s*\"0000000141d008086b3acc3602d3f58ca1b3b4\".*?AMPLITUDE_TAILS\s*=\s*\{.*?\(0x5,\s*0xA,\s*160,\s*136\):\s*\"0000000141d008086b\".*?\(0x5,\s*0xA,\s*136,\s*160\):\s*\"0000000141d008086b\".*?\(0x5,\s*0xA,\s*160,\s*160\):\s*\"0000000141d008086b7fff\".*?\(0x6,\s*0x9,\s*96,\s*160\):\s*\"0000000141d008086b7ffeef\".*?\(0x9,\s*0x6,\s*160,\s*96\):\s*\"0000000141d008086b7ade\".*?\(0xC,\s*0x3,\s*96,\s*160\):\s*\"0000000141d008086bbaff\".*?expected_u\s*=\s*cb_mask\.bit_count\(\) \* 8 \* abs\(cb_value - 128\).*?expected_v\s*=\s*cr_mask\.bit_count\(\) \* 8 \* abs\(cr_value - 128\).*?CABAC P16x16 cross-plane chroma-AC gate promoted.*?orthogonal axis-pair.*?mixed-sign.*?high-amplitude Cb/Cr guards strict-decode",
    ),
    (
