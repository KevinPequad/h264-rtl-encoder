...
CHECKS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        r"cod_i_queue=-7.*?EXPECTED_HEADER_TAIL\s*=\s*0x6B.*?TAILS\s*=\s*\{.*?\(0x1,\s*0x1\):\s*\"0000000141d008086b3acbb8b517a9\".*?\(0x6,\s*0x9\):\s*\"0000000141d008086b3acc6941a435899e104b1c8a00\".*?\(0x9,\s*0x6\):\s*\"0000000141d008086b3acc68c9ae34745f6b095dfa00\".*?\(0x2,\s*0xF\):\s*\"0000000141d008086b3acbf4ded3f999944575ae9b\".*?\(0x4,\s*0x8\):\s*\"0000000141d008086b3acbdfe134e5\".*?\(0x8,\s*0x4\):\s*\"0000000141d008086b3acbe70e2692\".*?\(0xF,\s*0x2\):\s*\"0000000141d008086b3acc332499ec2488aa9fedd7\".*?\(0xF,\s*0xF\):\s*\"0000000141d008086b7acc\".*?\(0xE,\s*0x1\):\s*\"0000000141d008086b3acc3602d3f58ca1b3b4\".*?AMPLITUDE_TAILS\s*=\s*\{.*?\(0x5,\s*0xA,\s*160,\s*136\):\s*\"0000000141d008086b\".*?\(0x5,\s*0xA,\s*136,\s*160\):\s*\"0000000141d008086b\".*?\(0x5,\s*0xA,\s*160,\s*160\):\s*\"0000000141d008086b7fff\".*?\(0x6,\s*0x9,\s*96,\s*160\):\s*\"0000000141d008086b7ffeef\".*?\(0x9,\s*0x6,\s*160,\s*96\):\s*\"0000000141d008086b7ade\".*?\(0xC,\s*0x3,\s*96,\s*160\):\s*\"0000000141d008086bbaff\".*?\(0x2,\s*0xF,\s*160,\s*96\):\s*\"0000000141d008086b3aff7dbfbe\".*?expected_u\s*=\s*cb_mask\.bit_count\(\) \* 8 \* abs\(cb_value - 128\).*?expected_v\s*=\s*cr_mask\.bit_count\(\) \* 8 \* abs\(cr_value - 128\).*?CABAC P16x16 cross-plane chroma-AC gate promoted.*?bottom-row cross-corner.*?sparse\+dense right-column.*?mixed-sign.*?high-amplitude Cb/Cr guards strict-decode",
    ),
    (
...
def main() -> int:
    ...
    print("[PASS] CABAC chroma residual wiring preserves CBP, scan, context bases, edge-coded plane-local chroma AC CBF context selection, state-dispatch, category scheduling, decoded-plane sanity coverage, dense Cr strict-pass promotion, Cr sparse strict-pass promotion, promoted Cb/Cr chroma-AC mask lattices with exact plane-local SAD, promoted cross-plane Cb+Cr AC strict-decode coverage, dense Cb and both-plane AC strict-pass controls plus decoded-plane sanity, chroma AC per-plane block counters, Cr AC first-payload substitution coverage, Cb/Cr AC promoted shape/amplitude strict-decode coverage, Cb AC phase/polarity tail plus first-payload substitution coverage, Cr AC phase/polarity tail plus first-payload substitution coverage, and combined plus single-plane luma/chroma DC-only/AC residual strict-decode smoke coverage with locked current decoded-plane metrics")
    return 0
