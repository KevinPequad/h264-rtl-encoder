...
LABEL="tpc_8x8_fwd_inv_unit_${BIT_DEPTH}b"
...
rm -rf "$BUILD_DIR"
{
  ...
  OBJCACHE= "$VERILATOR" --cc --exe --build -j "${BUILD_JOBS:-1}" \
...
"$BUILD_DIR/Vh264_tpc_8x8_unit_top" +mode=fwd_inv +bit_depth="$BIT_DEPTH" >"$SIM_LOG" 2>&1
cat "$SIM_LOG"
