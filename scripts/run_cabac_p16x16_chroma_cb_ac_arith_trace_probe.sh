...
print("[PASS] CABAC P16x16 Cb-only chroma AC arithmetic trace probe locks strict-decoding top, split top+bottom, and bottom-pair masks, including early header debug state, P-slice emit tail/bit-buffer rows, residual output/emit byte chunks, stream tails, terminate pre-state, and decoded-plane SAD")
PY
