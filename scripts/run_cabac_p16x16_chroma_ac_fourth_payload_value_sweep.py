#!/usr/bin/env python3
"""Representative fourth-CABAC-payload value sweep for chroma AC.

The first-, second-, and third-payload sweeps showed broad-to-narrow decode-equivalence
classes around the `d0 08 08 6b eb ...` CABAC residual boundary.  This bounded
diagnostic keeps the RTL checkout unchanged and mutates only the fourth CABAC
payload byte for three representative currently-bad cases.  It locks that the
baseline fourth byte is also outside the strict expected-SAD class while the pass
sets remain narrow (unique for two representatives, non-unique for one).
"""
...
EXPECTED_PASS_RANGES = {
    "cb_mask1": "0xe5",
    "cr_mask3": "0x04-0x05,0x0f,0x12,0x29,0x30,0x37,0x40,0x42,0x48-0x4b,0x5e,0x61,0x6a,0x77,0x7a,0x7c,0x85-0x86,0x88,0x8c,0x95,0x9b,0x9d,0xa5,0xc3,0xe9,0xf7",
    "cbcr_sparse_pair": "0x7e",
}
...
def make_fixture(name: str, cb_mask: int, cr_mask: int) -> Path:
    ...
    path = data_dir / f"smoke_16x16_2f_cabac_p16x16_chroma_ac_fourth_payload_value_sweep_{name}.yuv"
    path.write_bytes(y0 + flat + flat + y1 + checker_chroma(cb_mask) + checker_chroma(cr_mask))
    print(f"[INFO] CHRAC_FOURTH_PAYLOAD_VALUE {name} fixture {path.relative_to(ROOT)} size={path.stat().st_size}")
    return path


def build_baseline_sim() -> Path:
    workspace = Path(stage_workspace("h264_cabac_chrac_fourth_payload_value_sweep_"))
    config = BuildConfig(
        width=WIDTH,
        height=HEIGHT,
        bit_depth=8,
    ...
    print(f"[INFO] CHRAC_FOURTH_PAYLOAD_VALUE workspace={workspace} sim={sim}")
    return sim


def run_case(sim: Path, name: str, fixture: Path) -> bytes:
    out_dir = ROOT / "output" / "cabac_chroma_ac_fourth_payload_value_sweep"
    ...
    for needle in ("cabac_p16x16_mbs=1", "cabac_chroma_ac_mbs=1"):
        if needle not in sim_text:
            raise SystemExit(f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} sim log missing {needle}")
...
def check_baseline_stream(stream: bytes, name: str) -> None:
    ...
    if len(stream) != expected_len:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} stream length {len(stream)}, expected {expected_len}"
        )
    if actual_tail != expected_tail_hex:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} final tail drifted:\n"
            f"  got      {actual_tail}\n"
            f"  expected {expected_tail_hex}"
        )


def payload_indices(stream: bytes, name: str) -> tuple[int, int, int, int]:
    last_start = stream.rfind(b"\x00\x00\x00\x01")
    if last_start < 0:
        raise SystemExit(f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} missing final Annex-B start code")
    ...
    if fourth_idx >= len(stream):
        raise SystemExit(f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} missing fourth CABAC payload byte")
    if stream[header_tail_idx] != EXPECTED_HEADER_TAIL:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} header tail 0x{stream[header_tail_idx]:02x}, "
            f"expected 0x{EXPECTED_HEADER_TAIL:02x}"
        )
    if stream[first_idx] != BASELINE_FIRST_PAYLOAD:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} baseline first payload 0x{stream[first_idx]:02x}, "
...
def decode_raw(stream: bytes) -> tuple[bytes, str]:
    with tempfile.NamedTemporaryFile(prefix="h264_chrac_fourth_value_sweep_", suffix=".h264", delete=False) as h264_tmp:
        h264_tmp.write(stream)
        h264_path = Path(h264_tmp.name)
    with tempfile.NamedTemporaryFile(prefix="h264_chrac_fourth_value_sweep_", suffix=".yuv", delete=False) as raw_tmp:
...
def check_case(sim: Path, name: str, spec: dict[str, int]) -> None:
    ...
    _, second_idx, third_idx, fourth_idx = payload_indices(stream, name)
    baseline_second_payload = spec["baseline_second_payload"]
    baseline_third_payload = spec["baseline_third_payload"]
    baseline_fourth_payload = spec["baseline_fourth_payload"]
    if stream[second_idx] != baseline_second_payload:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} baseline second payload 0x{stream[second_idx]:02x}, "
            f"expected 0x{baseline_second_payload:02x}"
        )
    if stream[third_idx] != baseline_third_payload:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} baseline third payload 0x{stream[third_idx]:02x}, "
            f"expected 0x{baseline_third_payload:02x}"
        )
    if stream[fourth_idx] != baseline_fourth_payload:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} baseline fourth payload 0x{stream[fourth_idx]:02x}, "
            f"expected 0x{baseline_fourth_payload:02x}"
        )

    pass_values: list[int] = []
    for value in range(256):
        mutated = bytearray(stream)
        mutated[fourth_idx] = value
    ...
    if len(pass_values) != spec["pass_count"]:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} pass-count {len(pass_values)}, expected {spec['pass_count']}: "
            f"{actual_ranges}"
        )
    if actual_ranges != EXPECTED_PASS_RANGES[name]:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} pass ranges drifted:\n"
            f"  got      {actual_ranges}\n"
            f"  expected {EXPECTED_PASS_RANGES[name]}"
        )
    if baseline_fourth_payload in pass_values:
        raise SystemExit(
            f"[FAIL] CHRAC_FOURTH_PAYLOAD_VALUE {name} baseline fourth payload 0x{baseline_fourth_payload:02x} "
            "unexpectedly strict-decodes"
        )
    print(
        f"[PASS] CHRAC_FOURTH_PAYLOAD_VALUE {name} pass_count={len(pass_values)} "
        f"baseline_second=0x{baseline_second_payload:02x} baseline_third=0x{baseline_third_payload:02x} "
        f"baseline_fourth=0x{baseline_fourth_payload:02x} "
...
def main() -> None:
    ...
    print(
        "[PASS] CABAC P16x16 chroma-AC fourth-payload value sweep locks narrow decode-equivalence classes "
        "for representative Cb-only, Cr-only, and sparse Cb+Cr misses; baseline fourth payload bytes stay "
