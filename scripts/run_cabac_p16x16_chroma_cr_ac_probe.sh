...
if name.startswith("cb_"):
...
elif name.startswith("cr_") or name in {"checker", "single_tl", "single_tr", "single_bl", "single_br"}:
    ...
    if u_sad == 0 or v_sad == 0:
        raise SystemExit(
            f"[FAIL] CR_AC {name} strict-pass decoded plane sanity expected both chroma planes to change, got U_SAD={u_sad} V_SAD={v_sad}"
        )
else:
...
run_strict_pass checker 'cabac_chroma_cb_ac_mbs=0 cabac_chroma_cr_ac_mbs=1' 'cabac_chroma_cb_ac_blocks=0 cabac_chroma_cr_ac_blocks=4'
...
echo "[PASS] CABAC P16x16 sparse chroma AC strict-decode blocker is narrowed: dense Cr, Cr single_tl/single_tr/single_bl/single_br, dense both-plane AC, and Cb mirror single_br strict-decode, while sparse Cb top/left mirrors remain signature-locked expected misses"
