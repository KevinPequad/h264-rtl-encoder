...
def first_payload_index(data: bytes, label: str) -> int:
    ...
    if last_start < 0:
        raise SystemExit(f"[FAIL] CB_AC_SHAPE {label} has no final Annex-B start code")
    header_tail_idx = last_start + 8
    first_idx = last_start + 9
    if first_idx >= len(data):
        raise SystemExit(f"[FAIL] CB_AC_SHAPE {label} has no first CABAC payload byte")
    if data[header_tail_idx] != 0x6B:
        raise SystemExit(
            f"[FAIL] CB_AC_SHAPE {label} header-tail byte 0x{data[header_tail_idx]:02x}, expected 0x6b"
        )
    if data[first_idx] != FIRST_CABAC_PAYLOAD:
        raise SystemExit(
            f"[FAIL] CB_AC_SHAPE {label} first CABAC payload 0x{data[first_idx]:02x}, expected 0xeb"
        )
    return first_idx


def decode_h264_bytes(data: bytes) -> tuple[bytes, str]:
    with tempfile.NamedTemporaryFile(prefix="h264_cabac_cb_ac_shape_sub_", suffix=".h264", delete=False) as h264_tmp:
        h264_tmp.write(data)
        h264_path = Path(h264_tmp.name)
    with tempfile.NamedTemporaryFile(prefix="h264_cabac_cb_ac_shape_sub_", suffix=".yuv", delete=False) as raw_tmp:
...
def assert_first_payload_substitutions(h264: Path, input_path: Path, block: int, pattern_name: str, test_name: str, cb_value: int) -> None:
    stream = h264.read_bytes()
    first_idx = first_payload_index(stream, f"block={block} pattern={test_name}")
    expected_u_sad = pattern_pixel_count(pattern_name) * abs(cb_value - 128)
    ...
    for label, value in PROMOTED_PAYLOADS:
        ...
        if err.strip():
            raise SystemExit(f"[FAIL] CB_AC_SHAPE {label} block={block} pattern={test_name} FFmpeg log {err.strip()!r}")
        if len(raw) != expected_bytes:
            raise SystemExit(
                f"[FAIL] CB_AC_SHAPE {label} block={block} pattern={test_name} decoded {len(raw)}/{expected_bytes}"
            )
        if raw[:frame_size] != src[:frame_size]:
            raise SystemExit(f"[FAIL] CB_AC_SHAPE {label} block={block} pattern={test_name} changed IDR reference")
        ...
        if u_sad != expected_u_sad or v_sad != 0:
            raise SystemExit(
                f"[FAIL] CB_AC_SHAPE {label} block={block} pattern={test_name} SAD U={u_sad} V={v_sad}, "
                f"expected U={expected_u_sad} V=0"
            )
        print(
            f"[PASS] CB_AC_SHAPE {label} block={block} pattern={test_name} promotes/stays strict "
            f"at {len(raw)}/{expected_bytes} with exact Cb-only U_SAD={u_sad}"
        )
...
for block, pattern_name, test_name, cb_value, expected_u_sad, expect_full, short_signature, expected_final_slice in ALL_CASES:
    ...
    if cb_value == 160:
        assert_first_payload_substitutions(h264, input_path, block, pattern_name, test_name, cb_value)
...
print("[PASS] CABAC P16x16 sparse Cb AC shape probe locks coefficient-shape-sensitive strict/miss partition, exact strict-pass Cb-only U_SAD values, exact final-slice tails, and high-amplitude checker/diagonal/axis first-payload substitution promotion under common generated byte eb; repair target is scoped CABAC first-payload/residual-tail arithmetic, not only top-row block placement, sparse CBF context selection, low-amplitude checker parity, or the P-slice boundary")
PY
