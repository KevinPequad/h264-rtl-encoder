// h264_intra_pred.v - Intra Prediction (vertical, horizontal, DC, and directional 4x4 modes)
// Chooses the best supported mode by minimum SAD against the original block.
// Outputs the selected prediction block, residual, and H.264 intra4x4 mode.

module h264_intra_pred #(
    parameter BIT_DEPTH = 8
) (
    input  wire                      clk,
    input  wire                      rst_n,
    input  wire                      start,
    output reg                       done,
    input  wire                      top_avail,
    input  wire                      left_avail,
    input  wire                      top_left_avail,
    input  wire [16*BIT_DEPTH-1:0]   orig_4x4,
    input  wire [BIT_DEPTH-1:0]      top_left,
    input  wire [4*BIT_DEPTH-1:0]    top_4,
    input  wire [4*BIT_DEPTH-1:0]    top_right_4,
    input  wire [4*BIT_DEPTH-1:0]    left_4,
    input  wire                      force_mode_en,
    input  wire [3:0]                force_mode,
    output reg  [16*BIT_DEPTH-1:0]   pred_4x4,
    output reg  [16*(BIT_DEPTH+1)-1:0] resid_4x4,
    output reg  [3:0]                pred_mode
);

    localparam BD  = BIT_DEPTH;
    localparam BD1 = BIT_DEPTH + 1;
    localparam SAD_W = BIT_DEPTH + 5;

    localparam [3:0] MODE_VERT = 4'd0;
    localparam [3:0] MODE_HOR  = 4'd1;
    localparam [3:0] MODE_DC   = 4'd2;
    localparam [3:0] MODE_DDL  = 4'd3;
    localparam [3:0] MODE_DDR  = 4'd4;
    localparam [3:0] MODE_VR   = 4'd5;
    localparam [3:0] MODE_HD   = 4'd6;
    localparam [3:0] MODE_VL   = 4'd7;
    localparam [3:0] MODE_HU   = 4'd8;

    localparam [BD-1:0] DC_DEFAULT = {1'b1, {(BD-1){1'b0}}};

    wire [BD-1:0] top_pix  [0:3];
    wire [BD-1:0] top_right_pix [0:3];
    wire [BD-1:0] left_pix [0:3];
    wire [BD-1:0] top_ext  [0:4];
    wire [BD-1:0] top_full [0:7];
    wire [BD-1:0] left_ext [0:4];

    reg [BD+2:0] sum_top_c;
    reg [BD+2:0] sum_left_c;
    reg [BD-1:0] dc_value_c;

    reg [16*BD-1:0] pred_v_c;
    reg [16*BD-1:0] pred_h_c;
    reg [16*BD-1:0] pred_d_c;
    reg [16*BD-1:0] pred_ddl_c;
    reg [16*BD-1:0] pred_ddr_c;
    reg [16*BD-1:0] pred_vr_c;
    reg [16*BD-1:0] pred_hd_c;
    reg [16*BD-1:0] pred_vl_c;
    reg [16*BD-1:0] pred_hu_c;
    reg [16*BD1-1:0] resid_v_c;
    reg [16*BD1-1:0] resid_h_c;
    reg [16*BD1-1:0] resid_d_c;
    reg [16*BD1-1:0] resid_ddl_c;
    reg [16*BD1-1:0] resid_ddr_c;
    reg [16*BD1-1:0] resid_vr_c;
    reg [16*BD1-1:0] resid_hd_c;
    reg [16*BD1-1:0] resid_vl_c;
    reg [16*BD1-1:0] resid_hu_c;
    reg [16*BD-1:0] best_pred_c;
    reg [16*BD1-1:0] best_resid_c;

    reg [SAD_W-1:0] sad_v_c;
    reg [SAD_W-1:0] sad_h_c;
    reg [SAD_W-1:0] sad_d_c;
    reg [SAD_W-1:0] sad_ddl_c;
    reg [SAD_W-1:0] sad_ddr_c;
    reg [SAD_W-1:0] sad_vr_c;
    reg [SAD_W-1:0] sad_hd_c;
    reg [SAD_W-1:0] sad_vl_c;
    reg [SAD_W-1:0] sad_hu_c;
    reg [SAD_W-1:0] best_sad_c;
    reg [3:0] best_mode_c;

    integer idx;
    integer row_idx;
    integer col_idx;
    integer flat_idx;

    reg [BD-1:0] orig_pix_c;
    reg [BD-1:0] pred_pix_c;
    reg [BD:0] abs_diff_c;
    reg signed [3:0] z_vr_c;
    reg signed [3:0] z_hd_c;
    reg [2:0] ddr_d_c;
    reg [2:0] vr_n_c;
    reg [2:0] hd_n_c;
    reg [3:0] hu_z_c;

    function automatic [BD-1:0] avg2_pix;
        input [BD-1:0] a;
        input [BD-1:0] b;
        reg [BD:0] sum;
        begin
            sum = {1'b0, a} + {1'b0, b} + {{BD{1'b0}}, 1'b1};
            avg2_pix = sum[BD:1];
        end
    endfunction

    function automatic [BD-1:0] avg3_pix;
        input [BD-1:0] a;
        input [BD-1:0] b;
        input [BD-1:0] c;
        reg [BD+1:0] sum;
        begin
            sum = {2'b00, a} + {1'b0, b, 1'b0} + {2'b00, c} + {{BD{1'b0}}, 2'd2};
            avg3_pix = sum[BD+1:2];
        end
    endfunction

    genvar gi;
    generate
        for (gi = 0; gi < 4; gi = gi + 1) begin : unpack_neighbors
            assign top_pix[gi]  = top_4[gi*BD +: BD];
            assign top_right_pix[gi] = top_right_4[gi*BD +: BD];
            assign left_pix[gi] = left_4[gi*BD +: BD];
        end
    endgenerate

    assign top_ext[0] = top_left;
    assign top_ext[1] = top_pix[0];
    assign top_ext[2] = top_pix[1];
    assign top_ext[3] = top_pix[2];
    assign top_ext[4] = top_pix[3];

    assign top_full[0] = top_pix[0];
    assign top_full[1] = top_pix[1];
    assign top_full[2] = top_pix[2];
    assign top_full[3] = top_pix[3];
    assign top_full[4] = top_right_pix[0];
    assign top_full[5] = top_right_pix[1];
    assign top_full[6] = top_right_pix[2];
    assign top_full[7] = top_right_pix[3];

    assign left_ext[0] = top_left;
    assign left_ext[1] = left_pix[0];
    assign left_ext[2] = left_pix[1];
    assign left_ext[3] = left_pix[2];
    assign left_ext[4] = left_pix[3];

    always @(*) begin
        sum_top_c = {(BD+3){1'b0}};
        sum_left_c = {(BD+3){1'b0}};
        for (idx = 0; idx < 4; idx = idx + 1) begin
            if (top_avail)
                sum_top_c = sum_top_c + {{3{1'b0}}, top_pix[idx]};
            if (left_avail)
                sum_left_c = sum_left_c + {{3{1'b0}}, left_pix[idx]};
        end

        if (top_avail && left_avail)
            dc_value_c = (sum_top_c + sum_left_c + {{(BD+1){1'b0}}, 3'd4}) >> 3;
        else if (top_avail)
            dc_value_c = (sum_top_c + {{(BD+1){1'b0}}, 2'd2}) >> 2;
        else if (left_avail)
            dc_value_c = (sum_left_c + {{(BD+1){1'b0}}, 2'd2}) >> 2;
        else
            dc_value_c = DC_DEFAULT;

        pred_v_c = {(16*BD){1'b0}};
        pred_h_c = {(16*BD){1'b0}};
        pred_d_c = {(16*BD){1'b0}};
        pred_ddl_c = {(16*BD){1'b0}};
        pred_ddr_c = {(16*BD){1'b0}};
        pred_vr_c = {(16*BD){1'b0}};
        pred_hd_c = {(16*BD){1'b0}};
        pred_vl_c = {(16*BD){1'b0}};
        pred_hu_c = {(16*BD){1'b0}};
        resid_v_c = {(16*BD1){1'b0}};
        resid_h_c = {(16*BD1){1'b0}};
        resid_d_c = {(16*BD1){1'b0}};
        resid_ddl_c = {(16*BD1){1'b0}};
        resid_ddr_c = {(16*BD1){1'b0}};
        resid_vr_c = {(16*BD1){1'b0}};
        resid_hd_c = {(16*BD1){1'b0}};
        resid_vl_c = {(16*BD1){1'b0}};
        resid_hu_c = {(16*BD1){1'b0}};

        sad_v_c = {SAD_W{1'b1}};
        sad_h_c = {SAD_W{1'b1}};
        sad_d_c = {SAD_W{1'b0}};
        sad_ddl_c = {SAD_W{1'b1}};
        sad_ddr_c = {SAD_W{1'b1}};
        sad_vr_c = {SAD_W{1'b1}};
        sad_hd_c = {SAD_W{1'b1}};
        sad_vl_c = {SAD_W{1'b1}};
        sad_hu_c = {SAD_W{1'b1}};

        for (row_idx = 0; row_idx < 4; row_idx = row_idx + 1) begin
            for (col_idx = 0; col_idx < 4; col_idx = col_idx + 1) begin
                flat_idx = row_idx * 4 + col_idx;
                orig_pix_c = orig_4x4[flat_idx*BD +: BD];

                pred_pix_c = top_avail ? top_pix[col_idx] : {BD{1'b0}};
                pred_v_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_v_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (top_avail)
                    sad_v_c = sad_v_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                pred_pix_c = left_avail ? left_pix[row_idx] : {BD{1'b0}};
                pred_h_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_h_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (left_avail)
                    sad_h_c = sad_h_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                pred_pix_c = dc_value_c;
                pred_d_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_d_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                sad_d_c = sad_d_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                if (row_idx == 3 && col_idx == 3)
                    pred_pix_c = avg3_pix(top_full[6], top_full[7], top_full[7]);
                else
                    pred_pix_c = avg3_pix(top_full[row_idx + col_idx], top_full[row_idx + col_idx + 1], top_full[row_idx + col_idx + 2]);
                pred_ddl_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_ddl_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (top_avail)
                    sad_ddl_c = sad_ddl_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                ddr_d_c = col_idx[2:0] - row_idx[2:0];
                if (col_idx > row_idx)
                    pred_pix_c = avg3_pix(top_ext[ddr_d_c - 3'd1], top_ext[ddr_d_c], top_ext[ddr_d_c + 3'd1]);
                else if (col_idx < row_idx) begin
                    ddr_d_c = row_idx[2:0] - col_idx[2:0];
                    pred_pix_c = avg3_pix(left_ext[ddr_d_c - 3'd1], left_ext[ddr_d_c], left_ext[ddr_d_c + 3'd1]);
                end else
                    pred_pix_c = avg3_pix(top_pix[0], top_left, left_pix[0]);
                pred_ddr_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_ddr_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (top_avail && left_avail && top_left_avail)
                    sad_ddr_c = sad_ddr_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                z_vr_c = {col_idx[2:0], 1'b0} - {2'b00, row_idx[1:0]};
                vr_n_c = col_idx[2:0] - row_idx[1:0];
                if (z_vr_c == 4'sd0 || z_vr_c == 4'sd2 || z_vr_c == 4'sd4 || z_vr_c == 4'sd6)
                    pred_pix_c = avg2_pix(top_ext[vr_n_c], top_ext[vr_n_c + 3'd1]);
                else if (z_vr_c == 4'sd1 || z_vr_c == 4'sd3 || z_vr_c == 4'sd5)
                    pred_pix_c = avg3_pix(top_ext[vr_n_c], top_ext[vr_n_c + 3'd1], top_ext[vr_n_c + 3'd2]);
                else if (z_vr_c == -4'sd1)
                    pred_pix_c = avg3_pix(left_pix[0], top_left, top_pix[0]);
                else
                    pred_pix_c = avg3_pix(left_ext[row_idx[2:0]], left_ext[row_idx[2:0] - 3'd1], left_ext[row_idx[2:0] - 3'd2]);
                pred_vr_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_vr_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (top_avail && left_avail && top_left_avail)
                    sad_vr_c = sad_vr_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                z_hd_c = {row_idx[2:0], 1'b0} - {2'b00, col_idx[1:0]};
                hd_n_c = row_idx[2:0] - col_idx[1:0];
                if (z_hd_c == 4'sd0 || z_hd_c == 4'sd2 || z_hd_c == 4'sd4 || z_hd_c == 4'sd6)
                    pred_pix_c = avg2_pix(left_ext[hd_n_c], left_ext[hd_n_c + 3'd1]);
                else if (z_hd_c == 4'sd1 || z_hd_c == 4'sd3 || z_hd_c == 4'sd5)
                    pred_pix_c = avg3_pix(left_ext[hd_n_c], left_ext[hd_n_c + 3'd1], left_ext[hd_n_c + 3'd2]);
                else if (z_hd_c == -4'sd1)
                    pred_pix_c = avg3_pix(left_pix[0], top_left, top_pix[0]);
                else
                    pred_pix_c = avg3_pix(top_ext[col_idx[2:0]], top_ext[col_idx[2:0] - 3'd1], top_ext[col_idx[2:0] - 3'd2]);
                pred_hd_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_hd_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (top_avail && left_avail && top_left_avail)
                    sad_hd_c = sad_hd_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                if (row_idx[0] == 1'b0)
                    pred_pix_c = avg2_pix(top_full[col_idx + row_idx[2:1]], top_full[col_idx + row_idx[2:1] + 1]);
                else
                    pred_pix_c = avg3_pix(top_full[col_idx + row_idx[2:1]], top_full[col_idx + row_idx[2:1] + 1], top_full[col_idx + row_idx[2:1] + 2]);
                pred_vl_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_vl_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (top_avail)
                    sad_vl_c = sad_vl_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};

                hu_z_c = {1'b0, col_idx[2:0]} + {row_idx[2:0], 1'b0};
                if (hu_z_c == 4'd0)
                    pred_pix_c = avg2_pix(left_pix[0], left_pix[1]);
                else if (hu_z_c == 4'd1)
                    pred_pix_c = avg3_pix(left_pix[0], left_pix[1], left_pix[2]);
                else if (hu_z_c == 4'd2)
                    pred_pix_c = avg2_pix(left_pix[1], left_pix[2]);
                else if (hu_z_c == 4'd3)
                    pred_pix_c = avg3_pix(left_pix[1], left_pix[2], left_pix[3]);
                else if (hu_z_c == 4'd4)
                    pred_pix_c = avg2_pix(left_pix[2], left_pix[3]);
                else if (hu_z_c == 4'd5)
                    pred_pix_c = avg3_pix(left_pix[3'd2], left_pix[3'd3], left_pix[3'd3]);
                else
                    pred_pix_c = left_pix[3'd3];
                pred_hu_c[flat_idx*BD +: BD] = pred_pix_c;
                resid_hu_c[flat_idx*BD1 +: BD1] = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                if (orig_pix_c >= pred_pix_c)
                    abs_diff_c = {1'b0, orig_pix_c} - {1'b0, pred_pix_c};
                else
                    abs_diff_c = {1'b0, pred_pix_c} - {1'b0, orig_pix_c};
                if (left_avail)
                    sad_hu_c = sad_hu_c + {{(SAD_W-(BD+1)){1'b0}}, abs_diff_c};
            end
        end

        best_mode_c = MODE_DC;
        best_sad_c = sad_d_c;
        best_pred_c = pred_d_c;
        best_resid_c = resid_d_c;

        if (top_avail && (sad_v_c < best_sad_c)) begin
            best_mode_c = MODE_VERT;
            best_sad_c = sad_v_c;
            best_pred_c = pred_v_c;
            best_resid_c = resid_v_c;
        end

        if (left_avail && (sad_h_c < best_sad_c)) begin
            best_mode_c = MODE_HOR;
            best_sad_c = sad_h_c;
            best_pred_c = pred_h_c;
            best_resid_c = resid_h_c;
        end

        if (top_avail && left_avail && top_left_avail && (sad_ddr_c < best_sad_c)) begin
            best_mode_c = MODE_DDR;
            best_sad_c = sad_ddr_c;
            best_pred_c = pred_ddr_c;
            best_resid_c = resid_ddr_c;
        end

        if (top_avail && (sad_ddl_c < best_sad_c)) begin
            best_mode_c = MODE_DDL;
            best_sad_c = sad_ddl_c;
            best_pred_c = pred_ddl_c;
            best_resid_c = resid_ddl_c;
        end

        if (top_avail && left_avail && top_left_avail && (sad_vr_c < best_sad_c)) begin
            best_mode_c = MODE_VR;
            best_sad_c = sad_vr_c;
            best_pred_c = pred_vr_c;
            best_resid_c = resid_vr_c;
        end

        if (top_avail && left_avail && top_left_avail && (sad_hd_c < best_sad_c)) begin
            best_mode_c = MODE_HD;
            best_sad_c = sad_hd_c;
            best_pred_c = pred_hd_c;
            best_resid_c = resid_hd_c;
        end

        if (top_avail && (sad_vl_c < best_sad_c)) begin
            best_mode_c = MODE_VL;
            best_sad_c = sad_vl_c;
            best_pred_c = pred_vl_c;
            best_resid_c = resid_vl_c;
        end

        if (left_avail && (sad_hu_c < best_sad_c)) begin
            best_mode_c = MODE_HU;
            best_sad_c = sad_hu_c;
            best_pred_c = pred_hu_c;
            best_resid_c = resid_hu_c;
        end

        if (force_mode_en) begin
            case (force_mode)
                MODE_VERT: begin
                    best_mode_c = MODE_VERT;
                    best_pred_c = pred_v_c;
                    best_resid_c = resid_v_c;
                end
                MODE_HOR: begin
                    best_mode_c = MODE_HOR;
                    best_pred_c = pred_h_c;
                    best_resid_c = resid_h_c;
                end
                MODE_DDL: begin
                    best_mode_c = MODE_DDL;
                    best_pred_c = pred_ddl_c;
                    best_resid_c = resid_ddl_c;
                end
                MODE_DDR: begin
                    best_mode_c = MODE_DDR;
                    best_pred_c = pred_ddr_c;
                    best_resid_c = resid_ddr_c;
                end
                MODE_VR: begin
                    best_mode_c = MODE_VR;
                    best_pred_c = pred_vr_c;
                    best_resid_c = resid_vr_c;
                end
                MODE_HD: begin
                    best_mode_c = MODE_HD;
                    best_pred_c = pred_hd_c;
                    best_resid_c = resid_hd_c;
                end
                MODE_VL: begin
                    best_mode_c = MODE_VL;
                    best_pred_c = pred_vl_c;
                    best_resid_c = resid_vl_c;
                end
                MODE_HU: begin
                    best_mode_c = MODE_HU;
                    best_pred_c = pred_hu_c;
                    best_resid_c = resid_hu_c;
                end
                default: begin
                    best_mode_c = MODE_DC;
                    best_pred_c = pred_d_c;
                    best_resid_c = resid_d_c;
                end
            endcase
        end
    end

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            done <= 1'b0;
            pred_4x4 <= {(16*BD){1'b0}};
            resid_4x4 <= {(16*BD1){1'b0}};
            pred_mode <= MODE_DC;
        end else begin
            done <= 1'b0;
            if (start) begin
                pred_4x4 <= best_pred_c;
                resid_4x4 <= best_resid_c;
                pred_mode <= best_mode_c;
                done <= 1'b1;
            end
        end
    end

endmodule
