module h264_luma_dc #(
    ...
    generate
        for (gi = 0; gi < 16; gi = gi + 1) begin : unpack_in
            assign din[gi] = $signed(dc_in_flat_reg[gi*CW +: CW]);
        end
    endgenerate
    ...
    function [15:0] fwd_quant_dc;
        ...
        begin
            sign = val[CW+4];
            abs_val = sign ? (~val + {{(CW+4){1'b0}}, 1'b1}) : val;
            product = ({{16{1'b0}}, abs_val} * (get_mf() >> 1)) + F_INTRA;
            level = product[QBITS +: 16];
            fwd_quant_dc = sign ? (~level + 16'd1) : level;
        end
    endfunction
    ...
    // The forward 4x4 luma-DC transform uses a final rounded divide-by-2,
    // while the inverse path does not. Keep the unshifted Hadamard outputs
    // for inverse scaling and derive rounded forward terms separately.
    wire signed [CW+3:0] f00 = (h00 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f01 = (h01 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f02 = (h02 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f03 = (h03 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f10 = (h10 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f11 = (h11 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f12 = (h12 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f13 = (h13 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f20 = (h20 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f21 = (h21 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f22 = (h22 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f23 = (h23 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f30 = (h30 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f31 = (h31 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f32 = (h32 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    wire signed [CW+3:0] f33 = (h33 + $signed({{(CW+3){1'b0}}, 1'b1})) >>> 1;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            dc_out_flat <= 256'd0;
            do_inverse_reg <= 1'b0;
            dc_in_flat_reg <= {(16*CW){1'b0}};
        end else begin
            done <= 1'b0;
            case (state)
                S_IDLE: begin
                    if (start) begin
                        do_inverse_reg <= do_inverse;
                        dc_in_flat_reg <= dc_in_flat;
                        state <= S_EXEC;
                    end
                end

                S_EXEC: begin
                    if (!do_inverse_reg) begin
                        dc_out_flat[0*16 +: 16] <= fwd_quant_dc(f00);
                        dc_out_flat[1*16 +: 16] <= fwd_quant_dc(f01);
                        dc_out_flat[2*16 +: 16] <= fwd_quant_dc(f02);
                        dc_out_flat[3*16 +: 16] <= fwd_quant_dc(f03);
                        dc_out_flat[4*16 +: 16] <= fwd_quant_dc(f10);
                        dc_out_flat[5*16 +: 16] <= fwd_quant_dc(f11);
                        dc_out_flat[6*16 +: 16] <= fwd_quant_dc(f12);
                        dc_out_flat[7*16 +: 16] <= fwd_quant_dc(f13);
                        dc_out_flat[8*16 +: 16] <= fwd_quant_dc(f20);
                        dc_out_flat[9*16 +: 16] <= fwd_quant_dc(f21);
                        dc_out_flat[10*16 +: 16] <= fwd_quant_dc(f22);
                        dc_out_flat[11*16 +: 16] <= fwd_quant_dc(f23);
                        dc_out_flat[12*16 +: 16] <= fwd_quant_dc(f30);
                        dc_out_flat[13*16 +: 16] <= fwd_quant_dc(f31);
                        dc_out_flat[14*16 +: 16] <= fwd_quant_dc(f32);
                        dc_out_flat[15*16 +: 16] <= fwd_quant_dc(f33);
                    end else begin
